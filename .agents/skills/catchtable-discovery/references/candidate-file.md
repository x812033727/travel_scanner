# 候選檔 `candidates.json`：欄位與去向

一批一個目錄 `apps/api/app/foods/data/catchtable/<batch-id>/`，裡面四個檔：

| 檔 | 誰寫 | 用途 |
| --- | --- | --- |
| `rankings.json` | 本機瀏覽器片段 | 榜頁抄下來的 alias 清單，原樣保存，當證據 |
| `candidates.json` | 研究者（人或代理） | 每家店的查證結果；**這是交接檔**，每寫完一家就存 |
| `merchants.json` | `scripts/build_batches.py` | `import-trend-merchants --file` 的輸入 |
| `platform-reviews.json` | `scripts/build_batches.py`（要先有 worklist） | `apply-food-platform-reviews --file` 的輸入 |

腳本只擋「會讓整個匯入檔被拒」的錯；真正的規則在 `apps/api/app/foods/trend_import.py`
與 `apps/api/app/foods/platform_review_import.py`，兩邊都是一筆壞掉整檔拒收。
...
| `district_key` | 該城市 `apps/api/app/foods/area_catalog.py` 的 `key`（例：`myeongdong`、`hongdae`、`dongdaemun`、`gangnam`）；對不到就 `null`，匯入器遇到不存在的商圈會整筆跳過 |
| `name_zh` | 官方頁有中文名才用官方的；沒有就寫「韓文店名」本身，不猜漢字 |
| `name_en` | 拉丁字母；沒有就省略 |
| `local_name` | 韓文店名，含分店名（`본점`、`성수점`） |
| `address_local` | 只在來源頁印了地址時填；否則省略 |
| `category_slugs` | 1 到 3 個，`apps/api/app/foods/category_catalog.py` 的 slug；第一個是主分類 |

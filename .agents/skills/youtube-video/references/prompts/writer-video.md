...
- Chapters: at least 3, the first scene has one, each at least 10 seconds. Chapter names are what a viewer would search for (a question or a noun), never 「第一部分」.
- `youtube.title` at most 100 characters, no angle brackets; `youtube.description` is the body only (two opening lines that say what the video answers and for whom, then the substance). Chapters, the article link and the references are appended by the tool; do not write them. `tags` total at most 500 characters. `video_id` stays null.

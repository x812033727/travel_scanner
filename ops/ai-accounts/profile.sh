...
#
# In an interactive root shell, `claude` and `codex` use the account marked as default on
# /admin/ai-accounts. The account is resolved when the command starts, so a session keeps
# its account even if the default changes while it runs. `claude-a` … `codex-e` pick one
# explicitly. Scripts that call the CLIs by path, and shells that never read this file,
# get account A through the /root/.claude and /root/.codex links.

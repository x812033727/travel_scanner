...
座標由一個代理照 skill `references/admin.md` 的順序找（官方頁 JSON-LD 或頁面自己的地圖資料 → OSM 店家／建物節點；Naver、Google、Kakao 一律不用），
25 家找到 22 家、3 家沒有。Naver 精準頁照第一批的分工：站主在自己的 Naver 地圖依地址挑店、貼 `naver.me` 短網址，session 只讀轉址取 id。
公開 API（zh-TW）操作前：首爾 41 家／CatchTable 按鈕 13 顆，釜山 15 家／0 顆。

**結果（2026-09-23 09:5x UTC）**：25 家裡 **22 家已公開**——公開 API（zh-TW）首爾 41 → 54 家、釜山 15 → 24 家；CatchTable 訂位按鈕首爾 13 → 22 顆以上（公開 API 一頁最多 50 筆，`seoul-allaprima` 排在 50 筆之外，後台與 worklist 都是 approved／verified）、釜山 0 → 9 顆；四種 `X-Travel-Locale`（zh-TW／zh-CN／ja／ko）的 CatchTable 網址前綴分別是 `/zh-TW/`、`/zh-CN/`、`/ja-JP/`、無前綴。站主 2026-09-23 貼來 25 個 `naver.me` 短網址（MD 檔裡的搜尋連結在站主那邊打不開，站主自己搜的），25 個 id 互不重複、也不與目錄 94 列的任何 Naver id 撞號。沒有座標的 3 家（`seoul-haechen-obu`、`busan-jejugan-seomyun`、`busan-solsot-gwangan`）只存了 Naver 網址，維持 pending。

**座標已寫入 22 家**（站主同意後，在站主登入的面板裡用「編輯地點與來源」逐家儲存緯度、經度、座標來源類型、座標來源網址；地圖比對狀態留「待驗證」、審核留 pending，等 Naver 精準頁一起驗證）。後台清單一次只顯示一個目的地的待審列，首爾寫完要把目的地篩選切到釜山。

| slug | 韓文店名 | 座標 | 座標來源 | 依據 | Naver 精準頁 | 核准 |
| --- | --- | --- | --- | --- | --- | --- |
| `seoul-daowl` | 다올 숯불구이 명동점 | 37.563179, 126.983962 | `official_tourism` · [korean.visitkorea.or.kr/detail/ms_detail.do?cotid=0c2a1432-e](https://korean.visitkorea.or.kr/detail/ms_detail.do?cotid=0c2a1432-eab4-4fe1-a19e-c7669b865053) | KTO 대한민국 구석구석 店家頁（來源頁）本身沒有 JSON-LD，座標取自該頁地圖所用的 KTO 內容 API（POST /call cmd=TOUR_CONTENT_BODY_DETAIL，回傳 title 다올 숯불구이、addr1 명동8길 8-11 (명동2가)、mapX/mapY）；OSM node/13851761854「다올」amenity=restaurant（addr 명동8길 8-10, 명동2가, 2층）與此點相距不到 1 公尺，同巷 8-11 門牌的 OSM 節點（삼보식당、스페이스）在 20 公尺內，核對通過。 | `1220312094`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/FsRsf0ut`（只讀 307 轉址目標） | 2026-09-23 09:54 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `seoul-schedule-seongsu` | 스케줄 성수 | 37.544462, 127.055414 | `merchant_official` · [schedule-seongsu.com/ko](https://schedule-seongsu.com/ko) | 官網 schedule-seongsu.com/ko 的 JSON-LD Restaurant geo（37.5444617／127.0554135）；OSM node/10711556326「스케줄 성수」amenity=restaurant、addr 아차산로 104（37.5444425／127.0554581）相距約 4 公尺。 | `1481829719`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/FMcgsGsk`（只讀 307 轉址目標） | 2026-09-23 09:54 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `seoul-myeonseoul` | 면서울 | 37.524126, 127.038982 | `admin_verified` · [www.openstreetmap.org/node/13946797500](https://www.openstreetmap.org/node/13946797500) | tasteofseoul 頁沒有 JSON-LD geo；Nominatim「선릉로 805」命中 OSM node/13946797500「윤서울」（amenity=restaurant，addr:housenumber 805 선릉로 신사동）——這不是면서울本身，是同一門牌（선릉로 805，即 W빌딩）裡的另一家餐廳，附近建物 way 都沒有地址標籤，只能以同門牌節點代表，請站主留意。 | `1881172296`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/FHlgB6ia`（只讀 307 轉址目標） | 2026-09-23 09:54 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `seoul-evett` | 에빗 | 37.523429, 127.036264 | `admin_verified` · [www.openstreetmap.org/way/358294599](https://www.openstreetmap.org/way/358294599) | tasteofseoul 與 restaurantevett.com 都沒有 JSON-LD geo；Nominatim「도산대로45길 10-5」命中同門牌的 shop node（addr 「10-5, LS빌딩 2층」，37.5234348／127.0362062），再以「LS빌딩 도산대로45길」查到該建物 OSM way/358294599（building=commercial，名 LS빌딩），兩者相距約 5 公尺，採建物中心。 | `1838135293`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/F2Zuq8j6`（只讀 307 轉址目標） | 2026-09-23 09:54 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `seoul-7th-door` | 세븐스도어 | 37.522402, 127.054329 | `admin_verified` · [www.openstreetmap.org/way/897178311](https://www.openstreetmap.org/way/897178311) | tasteofseoul 與 7thdoor.kr 都沒有 JSON-LD geo；Nominatim「학동로97길 41」命中 OSM way/897178311（building，addr:housenumber 41 학동로97길 청담동）與同門牌餐廳 node/13946798652，採同門牌建物中心。 | `1100141122`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/52R5Lkod`（只讀 307 轉址目標） | 2026-09-23 09:54 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `seoul-choidot` | 쵸이닷 | 37.524449, 127.046552 | `admin_verified` · [www.openstreetmap.org/node/13946799117](https://www.openstreetmap.org/node/13946799117) | VisitKorea 專題頁 vcontsId=214935 無 JSON-LD geo；Nominatim「도산대로 457」命中 OSM node/13946799117「쵸이닷」amenity=restaurant、addr:housenumber 457 도산대로 청담동（同址另有 앙스돔빌딩 的 spa 節點），即店家本身。 | `280965665`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/55rmarQu`（只讀 307 轉址目標） | 2026-09-23 09:55 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `seoul-confier` | 콘피에르 | 37.556096, 126.975315 | `admin_verified` · [www.openstreetmap.org/node/13851761335](https://www.openstreetmap.org/node/13851761335) | VisitKorea 專題頁 vcontsId=205615 無 geo；confier.kr 的 JSON-LD geo 是模板佔位值（地址寫「강남구 예시로 123」、座標 37.5665／126.978 在市廳）不可用；Nominatim「서울 중구 세종대로 14」命中 OSM node/13851761335「콘피에르(Confier)」amenity=restaurant、addr:housenumber 14 세종대로 봉래동2가（그랜드센트럴），即店家本身。 | `1343969288`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/FN7XPaLJ`（只讀 307 轉址目標） | 2026-09-23 09:55 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `seoul-vinho` | 빈호 | 37.518395, 127.038002 | `admin_verified` · [www.openstreetmap.org/node/13805593915](https://www.openstreetmap.org/node/13805593915) | restaurantvinho.kr 沒有 JSON-LD geo；Nominatim「학동로43길 38」命中一排同門牌 OSM 節點，選 node/13805593915（shop=hairdresser 미니스 헤어 부띠크，addr「38, 지상1층 156호,157호,158호」）——與빈호的「논현웰스톤 1층 162호」同棟同層、房號相鄰；OSM 找不到빈호本身或名為 논현웰스톤 的建物 way（旁邊 way/1175296470 building=apartments 無地址標籤）。 | `1704712793`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/xxY29QGQ`（只讀 307 轉址目標） | 2026-09-23 09:55 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `seoul-kwonsooksoo` | 권숙수 | 37.524375, 127.043577 | `official_tourism` · [english.visitkorea.or.kr/svc/whereToGo/locIntrdn/rgnContents](https://english.visitkorea.or.kr/svc/whereToGo/locIntrdn/rgnContentsView.do?vcontsId=193236) | VisitKorea 英文站店家頁「Kwonsooksoo (권숙수)」JSON-LD geo；Nominatim「압구정로80길 37」命中同門牌 OSM node/13946797370（amenity=restaurant 마코토，addr 37 압구정로80길 청담동，37.5243756／127.0435776）相距不到 1 公尺；medical.visitseoul 頁的地圖中心 37.524385／127.043551 也一致。 | `37020894`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/xOxHBfeL`（只讀 307 轉址目標） | 2026-09-23 09:55 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `seoul-bongsanjeongyuk` | 봉산정육 홍대본점 | 37.552924, 126.921137 | `merchant_official` · [www.korean-bbq-hongdae.kr/ko/visit](https://www.korean-bbq-hongdae.kr/ko/visit) | 官網 korean-bbq-hongdae.kr/ko/visit 的 JSON-LD Restaurant geo（37.5529244／126.9211367）；OSM node/13946953021「봉산정육 홍대본점」amenity=restaurant、addr 양화로16길 30 서교동（37.5529313／126.9211466）與同門牌建物 way/300491082 相距約 1 公尺。 | `2032494208`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/5JGZ9yD5`（只讀 307 轉址目標） | 2026-09-23 09:55 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `seoul-haechen-obu` | 해천어부 | — | — | 官方頁沒有座標、OSM 沒有這個門牌或這棟樓（Nominatim 只到路段中心）；未填 | `1295127958`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/5A3pflFe`（只讀 307 轉址目標） | 未核准：沒有耐久座標（Naver 網址已存、地圖狀態待驗證、pending） |
| `seoul-allaprima` | 알라프리마 | 37.513551, 127.026025 | `admin_verified` · [www.openstreetmap.org/node/13946797486](https://www.openstreetmap.org/node/13946797486) | access.visitkorea 食物頁無座標，KTO 英文頁 vcontsId=67130 的 geo 是舊址不採用；Nominatim「학동로17길 13」命中 OSM node/13946797486「알라프리마(Alla prima)」amenity=restaurant、addr:housenumber 13 학동로17길 논현동（另一舊節點 node/6842862785「Alla Prima」相距約 6 公尺），即店家本身的新址。 | `36893601`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/FZ27VVJJ`（只讀 307 轉址目標） | 2026-09-23 已驗證＋核准＋啟用；CatchTable 訂位按鈕已出現 |
| `seoul-tonguidong-kukbingwan` | 통의동 국빈관 | 37.576629, 126.973822 | `official_tourism` · [korean.visitkorea.or.kr/detail/ms_detail.do?cotid=539c6805-1](https://korean.visitkorea.or.kr/detail/ms_detail.do?cotid=539c6805-191c-40ee-acfd-c31a5c6ca2f9) | KTO 대한민국 구석구석 店家頁（來源頁）沒有 JSON-LD，座標取自該頁地圖所用的 KTO 內容 API（cmd=TOUR_CONTENT_BODY_DETAIL，title 통의동 국빈관、addr1 자하문로2길 17-4、mapX/mapY）；Nominatim 只到 자하문로2길 路段中心（相距約 45 公尺），OSM 上該路有門牌 20、22 的建物 way/305411295、way/305411294 在路對面約 35 公尺處（17-4 是奇數側），OSM 沒有 17-4 本身，核對通過。 | `1025314822`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/FO9omQ5I`（只讀 307 轉址目標） | 2026-09-23 09:56 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `seoul-deepin-oksu` | 디핀옥수 | 37.542663, 127.012670 | `merchant_official` · [deepin-oksu.com/ko](https://deepin-oksu.com/ko) | 官網 deepin-oksu.com/ko 的 JSON-LD Restaurant geo（37.5426633／127.0126699）；Nominatim「독서당로 194」命中同門牌 OSM 餐廳節點 node/13946959562（37.5426827／127.0126588）與 node/12074556365，相距約 5 公尺。 | `1690484049`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/5r9zunOU`（只讀 307 轉址目標） | 2026-09-23 09:56 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `busan-cor-pasta-bar` | 코르 파스타 바 | 35.152650, 129.067670 | `official_tourism` · [www.visitbusan.net/index.do?menuCd=DOM_000000201002001000&uc](https://www.visitbusan.net/index.do?menuCd=DOM_000000201002001000&uc_seq=1820&lang_cd=ko) | Visit Busan 店家頁 uc_seq=1820（頁名 코르 파스타 바，주소 부산진구 동성로 25번길13 2F）內嵌 findPath_goal_lat/lng；OSM 沒有店家或門牌 13 的物件，Nominatim「동성로25번길 13」只到該路段中心 way/165593259（35.1527149／129.0672890），相距約 36 公尺，路段很短，核對通過（只到路段層級）。 | `1161690705`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/5iTdPGqh`（只讀 307 轉址目標） | 2026-09-23 09:52 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `busan-bandb-busan` | 본앤브레드 부산 | 35.160204, 129.165207 | `admin_verified` · [www.openstreetmap.org/node/4199814292](https://www.openstreetmap.org/node/4199814292) | bandb.co.kr 與 busanparadisehotel.co.kr 頁都沒有 JSON-LD geo；Nominatim「Paradise Hotel Busan」命中 OSM node/4199814292「파라다이스 호텔 부산」tourism=hotel、addr:housenumber 296（해운대해변로 296 即飯店本館，店在本館 B1），採飯店節點。 | `1121855131`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/F1rxkS5Y`（只讀 307 轉址目標） | 2026-09-23 09:52 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `busan-pungcheonman-haeundae` | 풍천만민물장어 해운대중동본점 | 35.162029, 129.167252 | `official_tourism` · [english.visitkorea.or.kr/svc/contents/contentsView.do?vconts](https://english.visitkorea.or.kr/svc/contents/contentsView.do?vcontsId=194517) | VisitKorea 英文站店家頁 JSON-LD geo；OSM 沒有店家或門牌 22 的物件，Nominatim「달맞이길 22」只到 달맞이길 路段中心 way/37399935（相距約 40 公尺），OSM 上 달맞이길 30 的建物 way/789212890 在東南約 100 公尺、해운대해변로298번길 47 的節點在南約 60 公尺，門牌順序一致，核對通過。 | `1834176128`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/GWe2Thy3`（只讀 307 轉址目標） | 2026-09-23 09:52 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `busan-ledorer` | 르도헤 | 35.155450, 129.146600 | `official_tourism` · [www.visitbusan.net/index.do?menuCd=DOM_000000201002001000&uc](https://www.visitbusan.net/index.do?menuCd=DOM_000000201002001000&uc_seq=1843&lang_cd=ko) | Visit Busan 店家頁 uc_seq=1843（頁名 르도헤，주소 해운대구 마린시티3로 37 213,214호）內嵌 findPath_goal_lat/lng；OSM way/781194218「한일오르듀」（CatchTable 地址寫的大樓名）中心 35.1555048／129.146638 相距約 7 公尺，Nominatim 마린시티3로 路段中心相距約 130 公尺，核對通過（uc_seq=1588 另一則同座標）。 | `1552906343`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/55rmLoOM`（只讀 307 轉址目標） | 2026-09-23 09:53 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `busan-sogonggan-dining` | 소공간 | 35.161690, 129.167370 | `official_tourism` · [www.visitbusan.net/index.do?menuCd=DOM_000000201002001000&uc](https://www.visitbusan.net/index.do?menuCd=DOM_000000201002001000&uc_seq=1846&lang_cd=ko) | Visit Busan 店家頁 uc_seq=1846 內嵌 findPath_goal_lat/lng 35.16169／129.16737；Nominatim「해운대해변로298번길 47」命中同門牌 OSM node/13964027070（tourism=gallery 갤러리토라，addr 47 해운대해변로298번길 중동，35.1616748／129.1673516）相距約 3 公尺，核對通過。 | `1621256204`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/IMyGJHhn`（只讀 307 轉址目標） | 2026-09-23 09:53 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `busan-jejugan-seomyun` | 제줏간 부산서면점 | — | — | 官方頁沒有座標、OSM 沒有這個門牌或這棟樓（Nominatim 只到路段中心）；未填 | `1142753247`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/G386sb6m`（只讀 307 轉址目標） | 未核准：沒有耐久座標（Naver 網址已存、地圖狀態待驗證、pending） |
| `busan-palate` | 팔레트 | 35.156513, 129.178340 | `official_tourism` · [www.visitbusan.net/kr/index.do?menuCd=DOM_000000201002002001](https://www.visitbusan.net/kr/index.do?menuCd=DOM_000000201002002001&uc_seq=2344&lang_cd=ko) | Visit Busan 三個 팔레트 條目中只有 uc_seq=2344（주소 달맞이길65번길 154, 3층）的內嵌座標落在 달맞이길；此點距 OSM 達맞이길65번길 路面（way/37403322）31 公尺，同路門牌 161-8、167、171 的建物 way/1292835136、1292835134、1292835135 在 78–116 公尺內（154 是對面偶數側），核對通過；1599／1811 的座標 35.131638／129.11977 在용호동，不用。 | `1259772157`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/xiqu5kSM`（只讀 307 轉址目標） | 2026-09-23 09:53 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `busan-yulling` | 율링 | 35.158867, 129.170600 | `official_tourism` · [www.visitbusan.net/kr/index.do?menuCd=DOM_000000201002002001](https://www.visitbusan.net/kr/index.do?menuCd=DOM_000000201002002001&uc_seq=1596&lang_cd=ko) | Visit Busan 店家頁 uc_seq=1596（頁名 율링，주소 달맞이길62번길 28 (중동)）內嵌 findPath_goal_lat/lng；OSM node/12200914903「Mipo Oceanside Hotel」（CatchTable 地址寫的 미포오션사이드호텔）相距約 25 公尺、建物 way/305381859 相距約 3 公尺，Nominatim 달맞이길62번길 路段中心相距約 190 公尺，核對通過。 | `1199260468`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/GzE920Tg`（只讀 307 轉址目標） | 2026-09-23 09:53 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `busan-arpkitchen` | 아르프 영도 | 35.093033, 129.041800 | `official_tourism` · [www.visitbusan.net/kr/index.do?menuCd=DOM_000000202003001000](https://www.visitbusan.net/kr/index.do?menuCd=DOM_000000202003001000&uc_seq=2183&lang_cd=ko) | Visit Busan 미식투어 頁 uc_seq=2183（頁名 아르프…빕구르망，주소 영도구 태종로99번길 35 1층）內嵌 findPath_goal_lat/lng；OSM 沒有店家或門牌 35 的物件，Nominatim「태종로99번길 35」只到該路段中心 way/117608554（35.0924609／129.0423889），相距約 85 公尺，核對通過（只到路段層級）。 | `1556516749`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/GubwiGT0`（只讀 307 轉址目標） | 2026-09-23 09:53 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |
| `busan-solsot-gwangan` | 솔솥 광안리점 | — | — | 官方頁沒有座標、OSM 沒有這個門牌或這棟樓（Nominatim 只到路段中心）；未填 | `1422399262`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/5apDOc6C`（只讀 307 轉址目標） | 未核准：沒有耐久座標（Naver 網址已存、地圖狀態待驗證、pending） |
| `busan-haegong` | 야키토리 해공 | 35.158237, 129.127120 | `official_tourism` · [www.visitbusan.net/kr/index.do?menuCd=DOM_000000201002002001](https://www.visitbusan.net/kr/index.do?menuCd=DOM_000000201002002001&uc_seq=1563&lang_cd=ko) | Visit Busan 店家頁 uc_seq=1563（頁名 야키토리해공，주소 민락본동로19번길 30-5 (민락동)）內嵌 findPath_goal_lat/lng，另一則 uc_seq=2362 的座標 35.158234／129.1271 相差 2 公尺；OSM 沒有店家或門牌 30-5 的物件，Nominatim「민락본동로19번길 30-5」只到該路段中心 way/658561313（35.1580409／129.1270324），相距約 23 公尺，核對通過（只到路段層級）。 | `1125056586`：站主 2026-09-23 在自己的 Naver 地圖依地址挑選、貼來的 `naver.me/5mI0JgZC`（只讀 307 轉址目標） | 2026-09-23 09:54 UTC 已驗證＋核准＋啟用（一次儲存，發布檢查通過）；CatchTable 訂位按鈕已出現 |

座標的三個判斷要說明：`seoul-daowl` 與 `seoul-tonguidong-kukbingwan` 的 KTO 韓文頁沒有 JSON-LD，座標是該頁地圖用的 KTO 內容 API 回傳值（mapX／mapY），
仍算官方頁自己的座標（`official_tourism`），並各用 OSM 核過（0.2 公尺、45 公尺）；`seoul-myeonseoul`（W빌딩內另一家餐廳的節點）與 `seoul-vinho`
（同棟同層、房號相鄰的店的節點）用的是**同門牌鄰居的 OSM 節點**，不是店家本身，`admin_verified`；`confier.kr` 的 JSON-LD geo 是模板佔位值、
KTO 英文頁的 `allaprima` geo 是舊址、Visit Busan `팔레트` 的 1599／1811 條目座標落在용호동——三個都排除。
...
- 這批 3 家沒有耐久座標的店（`seoul-haechen-obu` 수표로28길 11、`busan-jejugan-seomyun` 중앙대로691번가길 11、`busan-solsot-gwangan` 광남로 78）：Naver 網址已存、訂位平台列已是 verified，只差座標；OSM 有了門牌物件或觀光局頁出現座標就能公開，站主也可自行在後台填座標並註明依據。

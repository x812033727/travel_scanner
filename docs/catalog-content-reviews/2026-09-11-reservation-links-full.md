...
**結果：296 間沒有訂位按鈕的公開店家全部查過一次，另複查 5 間已公開的新加坡 Chope 頁，共 311 筆查核結果。其中 25 間店家會出現平台按鈕：11 間是可以線上訂位的精準分店頁，14 間是香港的 OpenRice 店家頁（只能電話訂位，依 2026-09-12 的決定也公開）。另外 23 筆是「平台上有這家店，但那一頁不能訂位」且不在香港，維持不公開。套用後全站有平台按鈕的店家會從 35 間變成 59 間——Song Fa 的 Chope 頁明寫不收訂位，它現有的按鈕會下架。**

| 國家 | 會公開 | 有頁面但不公開 | 無法確認 | 查無 |
| --- | --- | --- | --- | --- |
| JP | 2 | 1 | 1 | 92 |
| KR | 1 | 10 | 3 | 60 |
| TW | 2 | 3 | 1 | 47 |
...
香港那一欄的 16 筆裡，只有 2 筆（Arca Society 的 inline、FRANCIS 的 SevenRooms）能線上訂位，其餘 14 筆是 OpenRice 店家頁：頁面確認是本店，有營業時間、菜單與訂位電話，但沒有線上訂位功能。
...
- **香港 18 間裡有 14 間是「OpenRice 上查得到，但那一頁只能打電話訂位」**。OpenRice 頁面上的 Book 按鈕其實屬於下方的推薦餐廳廣告，本店自己沒有線上訂位。2026-09-12 決定這 14 筆仍然公開：OpenRice 是香港預設的美食平台，頁面有營業時間、菜單與訂位電話，對旅客仍然有用。這個例外只適用香港。
...
- `verified`：12 個支援平台之一的精準分店頁，店名與分店都對得上，而且頁面可以訂位。只有這種會出現在前台。
  - **香港例外（2026-09-12）**：香港的 OpenRice 店家頁只要確認是本店，即使只能電話訂位也算 `verified`。OpenRice 是當地預設的美食平台，頁面仍有營業時間、菜單與訂位電話。
- `disabled`：分店頁存在，但不能透過該平台訂位——頁面明寫不接受訂位、只有候位，或根本沒有訂位功能（香港的 OpenRice 除外）。不公開，改用店家官網實際連出的平台。

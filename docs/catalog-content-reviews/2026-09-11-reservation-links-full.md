...
**結果：296 間沒有訂位按鈕的公開店家全部查過一次，另複查 5 間已公開的新加坡 Chope 頁，共 311 筆查核結果。其中 11 間店家查到可以線上訂位的精準分店頁；36 筆是「平台上有這家店，但那一頁不能訂位」，依 2026-09-11 的決定不公開。套用後全站有訂位按鈕的店家會從 35 間變成 45 間——Song Fa 的 Chope 頁明寫不收訂位，它現有的按鈕會下架。**
...
- **inline 擋機器人**：台北 SIDOLI RADIO 與台南翰林茶館赤崁店的 inline 頁面多次被「按住不放」驗證擋下（沒有破解），維持「無法確認」。翰林另外在 OpenRice 有頁面，只能電話訂位，已記為停用。
- 台北春水堂信義店原本也在這一類，後來在低頻率下重試成功：頁面確認就是本店（台北信義新天地 A9 店 B1F、022723-9913，與官網信義店松壽路9號B1、02-27239913 相符），但明寫「目前尚不開放線上訂位」，已改記為停用。
- 這類封鎖是短時間大量開頁觸發的，不是內建瀏覽器打不開這些站：同一個瀏覽器隔一段時間單頁重試，OpenRice 的 15 筆與 inline 的 3 筆都順利查完。剩下這兩頁再重試時仍拿到同一組封鎖參考 ID，要再等一段時間，或由人在一般瀏覽器開啟確認。

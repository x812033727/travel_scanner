...
out) with the language and the hub's lead; and a `### Series` line per series or tutorial hub
from `GET /guides/series`, once each, under the first language (English first) that publishes
it. Both come from `getGuideTopics` and `getSeriesIndex`, and an outage leaves the sub-sections
out while the two hub lines stand.

Neither line carries a figure. The counts still choose the language, but this file is
regenerated per request and an answer engine may quote it back weeks later, by which time
"500 articles" is wrong. The same rule holds on every rendered page (2026-09-16): no article
counts and no lesson numbers anywhere a reader can see them, because both only ever grow.

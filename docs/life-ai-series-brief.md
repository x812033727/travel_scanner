...
指派給了**字尾關鍵字**（「翻譯 AI」「簡報 AI」這種把 AI 放在字尾的搜尋詞，見 `docs/ai-suffix-keywords.md`）時：
標題以它開頭，`description` 第一句與導言第一段各自然用到一次，h2 與其餘段落不重複；用字是中文後留半形空格再接大寫 AI。
...
區塊裡不能有 HTML、Markdown 語法或控制字元；`paragraph` 一段 ≤ 4,000 字；圖片與 hero 的 `alt` ≤ 200 字（一句話說畫了什麼，細節放 SVG 的 `<desc>`）、`caption` ≤ 300 字、表格格子 ≤ 300 字、`sources` 每筆 `title` ≤ 200 字（來源名稱加一句括號說明就好，細節寫進 notes.md）；文字裡的網址一律用 `link` 區塊，
...
- 官網對這個環境常回 403（Cloudflare）。已知可行的讀法：OpenAI help center 的文章網址後面加 `.json`
  （例如 `https://help.openai.com/en/articles/8792828-what-is-chatgpt-business.json`）會回完整頁面；其他站可用
  Wayback Machine 的最新快照（CDX 查 `statuscode:200` 再抓 `web/<時間戳>id_/<網址>`），或 WebSearch 限定官方網域取摘要交叉；
  `sources` 仍寫原始官網網址，`notes.md` 記下實際讀到的方式與快照日期。
...
Gemini 51 頁以 [系列維護說明](gemini-series/README.md) 為準，固定網址沿用總表，跨篇正文連結使用 `rich_paragraph`；提示詞、命令、JSON、TOML 與程式使用 `code` 區塊。新區塊的程式文字可包含 Markdown 或 HTML 語法，但只作為字面文字顯示，不執行。圖解需保持手機可讀，每篇 1,800–3,000 字，第 31 篇指令總表與總目錄例外。查證日使用實際查證日期，不能套用本指令的舊日期。

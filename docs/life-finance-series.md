...
- **語系：只寫 zh-TW。** 五語系共用一份 1,000 列的 sitemap（`SITEMAP_LIMIT`／`SITEMAP_GUIDE_ENTRY_LIMIT`），
  ...
  **2026-09-15 批次 03 落地後實測：986 列，只剩 14 列 headroom**（落地前是 966）。
  規劃這個系列時算的是 478 列，但 AI 系列這段期間推了好幾批，數字早就不是那個了。
  **批次 04–06（60 篇）放不進去，開工前一定要先做 `2026-09-14-sitemap-split-before-1000-rows`。**

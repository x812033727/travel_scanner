...
- **語系：只寫 zh-TW。** 五語系共用一份 1,000 列的 sitemap（`SITEMAP_LIMIT`／`SITEMAP_GUIDE_ENTRY_LIMIT`），
  ...
  **2026-09-15 實測：合併當天的 main 之後是 1,395 列——上限已經破了，不是快要破。**
  批次 03 的二十篇只佔其中 20 列（本分支併 main 之前是 986）；真正把數字推過去的是
  AI 系列與教學中心那幾批**五語系**內容，五語系包從 32 個變成 93 個。
  規劃這個系列時算的是 478 列，那個數字早就沒有意義了。
  **批次 04–06 開工前必須先做 `2026-09-14-sitemap-split-before-1000-rows`**——
  現在不是「留多少 headroom」的問題，是已經有幾百個已發布的語系頁進不了 sitemap。
...
批次 03 補充（保險與風險，全部是 YMYL 中最敏感的題材）：

- **示範條款的版本坑有四種形態，四個代理各踩到一種。** `law.fsc.gov.tw` 上是舊版，
  現行版在 `law.lia-roc.org.tw`：(1) 條次整段位移；(2) 條次沒動但內容改了；
  (3) 條次與文字都對、引文只抄前半句；(4) 同一條並列「（辦理電子商務適用）／（未辦理電子
  商務適用）」兩版，抄錯半邊。**只把條次數字加一，會得到一個號碼正確、內容過期的引用，
  比原本更難發現。唯一可靠的做法是逐條讀現行全文。** 細節與實測對照表在 brief 2.5。
- **產險不在 lia 站。** 那是**人壽**公會的系統，清單三十九筆全是人身保險。住宅火險這類產險的
  文件叫「**參考條款**」，在產險公會 `nlia.org.tw`。查不到就是站別選錯了，不要猜 FL 編號。
- **指派要說的是「去讀哪一份的現行全文」，不是「條文內容是什麼」。** 這一批我自己寫進指派的
  條文內容有五處是錯的（第 116 條項數、失能程度表、不存在的法規名稱、擇一而非各有限額、
  契約撤銷權抄到非電子商務版），全部由代理讀原文更正。協調者寫的具體內容也會錯。
- **協調者自己 curl 複驗是划算的。** 這一批六篇每篇都抽兩三條逐字核對，抓出兩處漏抄但書。
  週額度擋 API（含 WebFetch）但**不擋網路**，`curl` 那條路一直通。
- **`notes.md` 不是原代理寫的就要明說。** 額度中斷時協調者自己收尾的篇，開頭寫明這件事，
  並分「協調者重新核對通過的項目」與「未逐條重驗的部分」兩節，不替沒看過的查證背書。
- **法遵最敏感的題材要在指派裡把切角寫死，並劃出紅線。** 第 20 篇天生會滑向
  「如何對抗業務員」或「該問什麼才能買到好保單」，指派直接寫死成「把口語說法換回條款用語」
  並禁止對該職業的任何價值判斷，交回來的稿子零違規。連標題裡的「話術」都要換掉。

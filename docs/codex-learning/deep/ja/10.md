...
プロジェクトルートを見つけられない場合、指示の探索対象は現在のディレクトリだけです。Git を使わないこの例は codex-practice 直下から開始し、任意の子フォルダーからでも親のルールが見つかるとは考えないでください。リポジトリ内の比較は[ルールの適用範囲](article:codex-agents-md-scopes)、長い文書の分割は[ルールと参考資料の役割](article:codex-rules-and-context-files)で扱います。
...
| 今回の状況 | 必要な確認 | 確認する証拠 |
| --- | --- | --- |
| HTML の見出しだけを変更 | 画面幅、フォーカス、結果の報告 | 指定文字列と実際の画面。未実施項目も明記 |
| JavaScript を変更 | 既存のデータテストを実行 | 実際のコマンド、終了状態、テスト件数 |
| ツールを使えない | 理由を示し、合格とは書かない | 元のエラーと NOT RUN。結果を作らない |
...
```text 同じ Codex タスク。テストコマンドを別途確認

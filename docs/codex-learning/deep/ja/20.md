...
ローカルプレビューは Windows の py -m http.server 4173 --bind 127.0.0.1、他 OS は python3 を使います。URL を開いて教材リセット、Read/Build 追加、Read 完了、Completed 選択。期待 Read、実際 Build の順序を残します。
...
次の限定依頼を送り、既存テストと repro.mjs は固定します。修正後に古くなる故障コメント一行は消してよいが、他関数整理や画面・framework 変更はしません。失敗未確認なら先に確認させます。

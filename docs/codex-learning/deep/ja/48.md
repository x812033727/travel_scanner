...
教材検証は Node.js 検査の再現可能な入出力です。選択、自動適用、エージェントの遵守は自分の入口で観察し、スクリプト成功で代替しません。次の[スキル検証と修正](article:codex-skill-testing)は空入力、ID 重複、型不正、対象外依頼を加え、プログラムの正しさと適切な使用を分けます。図1は入力と契約、2は実行、3は報告確認です。

...
> 自分用の雛形フォルダーを作り、作業に合う欄を選び、観測値を記入し不要な手順を除きます。読む時間15分、実習20分。雛形は送信して初めて要求になる普通の文字で、新しい対話指令や自動実行プラグインではありません。[依頼の書き方](article:codex-prompting)と [Markdown](article:codex-markdown-basics)から始めます。

## 手順1：用途を分けて雛形を保存

実習ディレクトリ内に templates を作り、下記ファイルをエディターで追加します。Windows は .md.txt になっていないか確認、macOS は標準テキスト、Linux は UTF-8 で保存します。全 OS とも実習ルートを基準に同じ名前を使います。このフォルダーは自動読込みされず、追加導入、別サイトへのログイン、全域設定は不要です。
...
これは Small Steps 用の記入例で、どのプロジェクトにも無修正で使えるものではありません。副本のファイルと README の検証方法を確認します。可能性の相談なら [Plan モード](article:codex-plan-mode)で手順と未決事項を成果に変更します。実装なら編集範囲を明示し、停止と即完了を同時に求めません。
...
山括弧は記入欄であり端末構文ではありません。[修正実習](article:codex-debugging)なら「同名で ID が異なる二件」が「集計が変」より再現できます。Actual は観測だけを書き、未確定の原因を断定しません。エラーがなければ none observed と動作を記し、架空のトレースを作りません。
...
審査の比較元は現在の Git で解決できる必要があります。存在しなければ [Git と差分](article:codex-git-basics)を確認し main を仮定しません。引継ぎは現在地と状態を毎回更新します。以前の会話のコミット報告だけで現在の目録を未変更と判断せず、未コミット分と担当を残します。
...
設定、認証、能力、実呼出しを別記します。不要な認証は not required、未確認は not checked とします。設定ファイルだけで接続成功とせず true を予記しません。[Plugins](article:codex-plugins) や [MCP](article:codex-mcp) は今回の入口と主機を記録し、他機の同名設定を同一視しません。

## 間違い・復元・小実習

未記入の山括弧は送信前に < を検索し、記入または不要欄削除で直します。旧命令の流用は README と現在地の確認で防ぎます。読取り審査とついでの編集など競合する要求は一目的に絞ります。同名の既存雛形は別副本や Git 差分で保護し、復元も自分が変更した雛形に限定します。

実習では観測済みの不具合を bug.md、次の一歩を handoff.md に書き、元の作業を知らない人に渡します。場所、再現、未確認が分かれば合格です。主機やデータを推測させるならその欄を足し、全雛形を長くしません。空の原本を保持し作業ごとに副本を保存して、前回結果を既定値にしません。

原創教材であるこれらのファイルに実行権限はなく、アカウントも変えません。六つの読める雛形、記入済み報告と引継ぎ、特定した曖昧さ一件が検証成果です。モデルの特定表現は要求しません。次は [MCP 設定](article:codex-mcp)で接続雛形を実際の読取り試験に使います。

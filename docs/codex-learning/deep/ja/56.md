...
版タグは移動します。本例は確認済みの完全コミット SHA を保持します。確認時の Codex Action v1 は新しいコミットを指すため、本例を最新 v1 とはしません。更新前に新コミットの入力と権限仕様を読み、SHA 変更後に再検証します。Action 固定は全 runner 依存物や CLI の固定ではなく、実行版を記録します。
...
本例の既定待機規則では A 実行中、B 待機中に同群の C が来ると、B が C に置き換わり得ます。cancel-in-progress: false は A を保護し、全待機を保証しません。三つの有料実行を促すのでなく規則の演習です。採用前に各 run と artifact を照合し、複数待機が必要なら [GitHub concurrency](https://docs.github.com/en/actions/how-tos/write-workflows/choose-when-workflows-run/control-workflow-concurrency)を確認します。

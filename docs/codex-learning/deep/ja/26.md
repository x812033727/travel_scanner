...
```javascript ブラウザー DevTools Console：実習ページの読取り計測
(() => {
...
horizontalOverflow は false が期待値ですが、全ページ幅だけの検査です。操作性、可読性、フォーカスは次のキーボード・長い題名・保存の手順で確認します。Console を実際に開けない場合は未計測とし、期待値を観察結果に転記しません。 値の定義は [getComputedStyle](https://developer.mozilla.org/en-US/docs/Web/API/Window/getComputedStyle) と [scrollWidth](https://developer.mozilla.org/en-US/docs/Web/API/Element/scrollWidth) を参照します。

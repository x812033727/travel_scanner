...
やり直す場合は保存した handoff.md だけを未確認の初期状態に戻し、正常コードは保持します。四文書は教材内だけで全域設定に影響しません。実在とリンク、規則と歴史の区別、根拠あるテスト、欠落検出、未実行を成功にしない記録が完了条件です。独自教材と公式仕様、ローカル参照テストを、読者の Codex 実行と混同しません。

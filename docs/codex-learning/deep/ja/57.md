...
run_summary.py は subprocess.run(timeout=120) を使います。[Python 公式](https://docs.python.org/3/library/subprocess.html#subprocess.run)では、時間切れ時に直接の子プロセスを終了・待機してから TimeoutExpired を送出し、初期生成により指定秒数を超える場合があります。子孫、別主機、送信済み要求の全停止は証明しません。直接プロセスと未確認作業を分け、timeout 記録を保持します。

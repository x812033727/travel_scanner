...
並行で短縮できても統合には順序があります。二人完了は各分岐の完了にすぎず、両変更を使う同一画面は組合せ版で確認が必要です。今回は標題と頁脚の文書で交付を表し、全サイト試験とは呼びません。後で実際の建置・画面確認に適用します。
...
下記三ファイルを作り、標題と頁脚の初期値に末尾改行を残します。試験は未実装の新要件二つなので基準は意図した零通過・二失敗です。遠隔なしの独立教材であり、正式 main に説明なしの失敗を残すものではありません。
...
A は標題コピーの一文書だけ、B は頁脚コピーの一文書だけを指定値へ変えます。手動か、対応 Codex で全パス、分岐、唯一の編集対象、報告形式を明示して二代理へ依頼できます。参考検証は同等の明示編集でありモデル二実行とはしません。
...
| 作業 | 編集対象 | その段階の試験 |
| --- | --- | --- |
| A 標題 | heading.txt | 標題成功・頁脚未完 |
| B 頁脚 | footer.txt | 頁脚成功・標題未完 |
...
両者の書込み終了後、全状態と対象一ファイルだけを確認してコミットします。既コミットなら差分を確認し二重コミットしません。両 SHA を記録し、実協作では確認済みコミットを使うか更新を止め、動く対象で検証しません。
...
競合文書の両値と Git 標記を見て、今回は選択せずエラー・状態を残して abort します。二成功と未変更へ戻るはずです。開始前に未コミットがあると完全復元を保証できないので確認を省略しません。実解消では部分ごとに動作を選び、全て ours/theirs にしません。

...
> todo-summary の検査プログラムと、適切な依頼に対する Codex のスキル使用を確認します。読む時間15分、実習30分。[スキル資源](article:codex-skill-resources)の codex-skill-lab と4資源、対応 Node.js 22 以降、エディター、認証済み Codex を使います。最初に手動で元の3件が3／1／2になることを確認します。
...
| 状況 | 自然言語での依頼 | 確認点 |
| --- | --- | --- |
| 明示 | todo-summary を選び data/tasks.json を返信内だけで集計 | 資源読取り、実行、3／1／2、入力保持 |
| 適合 | 本機の待ちタスク JSON の全体・未完了・完了を数える | 選択の有無を記録し使用を偽らない |
| 対象外 | サイト背景を青にする計画のみ、変更しない | タスク集計器を実行しない |
| 必須資源なし | 仕様を改名して検証済み報告を明示依頼 | 欠落報告して停止し、仕様や結果を創作しない |
...
次の検収記録で未測定は未実施とし、期待値を埋めません。資源ならパス、誤選択なら description、集計ならコードを一つずつ直します。旧版と失敗例を保持し、影響テストと新規タスクを再実行します。主ファイルの変更だけで旧タスクの再読取りを証明できません。

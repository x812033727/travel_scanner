...
```sh Terminal: deliberately run from the wrong directory
cd .agents/skills/todo-summary
node scripts/count-tasks.mjs data/tasks.json
```

```sh After recording the failed exit: return to the practice root
cd ../../..
```

...
```sh System terminal: run after confirming the new rules-lab root
git init
...
```markdown File content: ui/AGENTS.md
...
```markdown File content: data/README.md
# Data notes
...
```sh System terminal: start a new CLI session from the lab root
codex --cd . --sandbox read-only
```

```text Natural-language prompt: enter in the new CLI session
...
```sh System terminal: after exiting the previous CLI, run from the root
...
```markdown File content: ui/AGENTS.override.md
...
```text Private observation record: one per start; not a CLI command

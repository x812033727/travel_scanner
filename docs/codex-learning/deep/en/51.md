...
This filled example targets the series' Small Steps practice app, not every repository. Verify the files in your own copy and align validation with its README. To discuss feasibility, use [Plan mode](article:codex-plan-mode) and change the delivery to a proposed sequence and unresolved decisions. For implementation, state the editable scope clearly rather than asking both to pause first and finish immediately.
...
Angle-bracket text marks fields to fill, not terminal syntax. In the [Bug exercise](article:codex-debugging), 'two tasks with the same title but different IDs' is more reproducible than 'the count is odd.' Actual contains observations, not an unverified diagnosis such as a broken database. If no error appeared, write none observed and describe the behavior and steps rather than inventing a stack trace.

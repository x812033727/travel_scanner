...
Obtain a normal read with the previous lesson's question, save the configuration, then add enabled = false to the existing practice section. Use Restart in desktop MCP settings, restart the IDE extension, or open a fresh CLI session. Merely asking again in the old conversation can reuse earlier content. This exercise deliberately disables your entry; it does not indicate a remote service outage.
...
```text Fresh interactive Codex CLI session: inspect tools
/mcp
```

The /mcp block is for interactive CLI. In desktop or IDE, confirm disabling in MCP servers settings, restart and inspect tool activity in a new task. Record that the client did not expose the tool; switching surfaces is unnecessary. If you compare surfaces, record each host and configuration location.

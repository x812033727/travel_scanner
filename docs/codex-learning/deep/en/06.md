...
```shell Terminal command: run in the codex-first-site folder
...
```powershell Windows PowerShell: start a local preview in the practice folder
...
```bash macOS/Linux terminal: start a local preview in the practice folder
...
```text Page input: paste into the todo field, then add
<img src=x>
```

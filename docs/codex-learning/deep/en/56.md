...
Version tags move. This example retains fully reviewed commit SHAs; the Codex Action v1 tag now points to a newer commit, so the example does not claim to use the latest v1. Review a new commit's inputs and permission behavior before updating and revalidating. Pinning an Action does not pin every runner dependency or the CLI; record actual runtime versions.

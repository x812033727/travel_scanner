...
```sh Terminal: version and baseline tests
node --version
...
```text Codex prompt: read-only project map
...
```powershell Windows terminal: local preview
py -m http.server 4173 --bind 127.0.0.1
```

```sh macOS/Linux terminal: local preview
...
```markdown Append to file: project-map.md
## Evidence and limits

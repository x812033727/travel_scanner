...
打開 core.mjs，選取 visibleTasks 整個函式，從命令面板找到 Add to Codex Thread，把選取範圍加入目前任務；名稱或可用命令以該版擴充套件選單為準。再開 core.test.mjs，讓測試也在上下文中。送出前查看輸入框的附件或檔案標記，確認是 core.mjs 和測試，不是只選到了相似名稱的另一份檔案。

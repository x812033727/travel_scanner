...
清理本次練習時只處理 safety-lab 中自己建立的檔案；保留需要的錯誤紀錄，不掃描整個家目錄或正式專案來搜尋金鑰。圖片截圖先檢查地址列、帳號、檔案路徑、終端機與通知區，使用[圖片協作](article:codex-browser-images)的署名與圖說規則。完成標準是能說明哪些來源可作資料、哪些操作已授權、忽略檔能做與不能做什麼，以及如何證明輸出未含本機假值。工具紀錄不足與尚未實測的入口都要留在驗收紀錄中。

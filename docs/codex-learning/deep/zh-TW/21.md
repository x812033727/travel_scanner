...
桌面版開啟本 Git 專案，先在 Review 面板選 Unstaged，確認看到剛改的 core.mjs，再於 Codex 輸入框使用 /review，核對所選差異。CLI 先從 codex-review-lab 的系統終端機執行 codex，確認工作目錄後，在互動輸入框使用 /review，選 Review uncommitted changes；不要把 /review 打在系統 shell。此處還沒 commit，只審某個已提交版本會漏掉 reverse。入口與範圍依[官方 Code review 文件](https://learn.chatgpt.com/docs/code-review)核對。

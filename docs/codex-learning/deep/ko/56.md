...
버전 태그는 이동합니다. 예제는 검토한 전체 커밋 SHA를 유지합니다. 확인 당시 Codex Action v1은 더 새 커밋을 가리키므로 최신 v1을 사용한다고 주장하지 않습니다. 업데이트 전에 새 커밋의 입력·권한 동작을 읽고 SHA 변경 후 다시 검증하세요. Action 고정은 모든 runner 의존성과 CLI 고정이 아니므로 실제 실행 버전도 기록합니다.

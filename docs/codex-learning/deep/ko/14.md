...
로컬 미리 보기에서 Read와 Build를 추가하고 Read를 완료합니다. All은 둘 다, Active는 Build, Completed는 Read입니다. 새로고침 후에도 Completed에 Read가 있고 빈 목록 필터도 오류가 없어야 합니다. OS별 실행은 [첫 프로젝트](article:codex-first-project)를 참고하세요. localhost 검증은 배포된 웹사이트가 아닙니다.

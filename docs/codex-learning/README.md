...
**60 篇五語深入稿與總目錄已於 2026-09-20 正式發布並通過公開網站驗收。** [正式發布紀錄](production-release-2026-09-20.md)分開列出 PR、CI、備份、匯入、部署與瀏覽器證據。系列對齊 Claude Code 教學中心的深度，保留既有 ID／網址，新增十單元、URL 篩選、指令索引、五語摘要與先備導覽；沒有短稿或待寫空篇。全文終審為 60/60，總目錄終審通過，修正紀錄見[最終編輯處置](evidence/final-editorial-resolution.json)。
...
本系列有一個總目錄與 60 篇獨立教學，每篇提供繁中、簡中、英文、日文、韓文，共 61 個內容包、305 份語言文件。路由為 `/{locale}/life/codex-learning-hub` 與既有 `/life/<slug>`。本機 Edge 以 360／390／1280px 完成 920 筆整頁報告（915 項頁面驗收與 5 次明確記錄的首次取文重試），另有 12 項目錄／實戰頁檢查及 6 項練習資料復原檢查；發布後另逐頁核對五語公開內容。

五篇代表稿與 A 至 J 十單元已完成定向補強及全文終審。先前被標為 changes-required 的 03、06、20、25–29、50–52、54 與總目錄已依精確問題修正；全文第三組證據亦已建立。代表篇真實 Codex 產品介面操作證據與實體手機／非 Windows 操作未納入本次實測，文內保留官方文件查證與示意圖標示。

## 內容與發布來源

[收尾進度安排](final-review-schedule.md)與[盤點紀錄](evidence/final-review-intake.json)保留製作階段的起點和門檻；最終結果見[正式發布紀錄](production-release-2026-09-20.md)。
...
- [驗證紀錄](evidence/)：本機內容／瀏覽器證據與正式公開驗收收據分開保存；只有正式發布紀錄可支持已上線狀態。

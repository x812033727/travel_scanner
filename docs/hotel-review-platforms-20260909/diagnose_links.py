...
SPEC = importlib.util.spec_from_file_location(
    "platform_link_diagnostic", Path(__file__).with_name("hotel_review_platforms_20260909.py")
)
...
async def main():
    baseline, _, pending = OP.baseline_data(sys.argv[1])
    ...
    async def check(entry):
        async with semaphore:
            ...
            health = await OP.CORE.check_hotel_link(OP.CORE.HotelLink(
                provider=options[entry["id"]]["provider"], url=url, evidence_url=url,
            ))
            result = {
                "id": entry["id"], "url": url, "health": health,
                "checked_at": OP.CORE.datetime.now(OP.CORE.UTC),
            }
            OP.CORE.emit(result)
            return result

    results = await asyncio.gather(*(check(e) for e in manifest["decisions"]))
    OP.CORE.save_new(output, {
        "tag": OP.TAG, "manifest_hash": OP.MANIFEST_HASH, "normal_guard_unchanged": True,
        "database_queries": 0, "mutations": 0, "quote_calls": 0, "clickout_calls": 0,
        "results": results,
    })

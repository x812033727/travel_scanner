...
MANIFEST_FILE_SHA = "39b81265da878c2ab252357650e976160b120b0fc329ffa6c7c305931d931fe0"
...
def run(phase):
    ...
    if len(products) != 40 or len(approved) != 279:
        raise ValueError("Unexpected baseline inventory")
    if phase != "before":
        results = [
            json.loads(line) for line in (HERE / "apply.ndjson").read_text().splitlines()
            if line.strip()
        ]
        expected = {f"{TAG}:option:{e['id']}" for e in manifest["decisions"]}
    ...
    for result in captures:
        ...
        expected_oids = {
            i for i, o in expected_options.items() if o["product_id"] in expected_pids
        }
        ...
        responses.append({
            ...
            "items": result["items"], "body_hash": CAPTURE.digest(result["items"]),
        })
    summary = {
    ...
    report = {
        ...
        "baseline_hash": BASELINE_HASH, "manifest_file_sha256": MANIFEST_FILE_SHA,
        "started_at": started, "completed_at": CAPTURE.now(),

...
範圍：migration `0055_analytics_event_names` 拿掉 `ck_analytics_event_name`，事件名的唯一真相移到 `analytics/service.py` 的 `EVENT_NAMES`（十七個）；新增 `analytics/context.py`（仿 `i18n.py` 的 contextvar，由 `RequestContextMiddleware` 綁定），因此 `usage/service.py` 這種拿不到 `Request` 的地方也送得出事件；新增 `record_event()` 直接寫 `analytics_events`，不經過訪客 ingest 的 IP／session 速率限制，而且量測永遠不會弄壞被量測的那個請求（所有查詢在 `no_autoflush` 底下、例外只記錄不上拋、未知事件名先擋下來）。第一版用 savepoint 做這件事，那反而是 bug 的來源：進入 savepoint 會先 flush 呼叫者還沒存的列，讓它的失敗落進這裡的 `try`，`POST /alerts` 因此在並行重複建立時回 500 而不是 409（#332 修掉）。伺服器端接上 `search_started`、`trip_created`（含 `source`，瀏覽器那兩處移除）、`place_added_to_trip`（三個探索面）、`ai_applied`、`optimize_applied`、`offer_attached`、`alert_created`、`share_created`、`share_forked`、`usage_charged`、`usage_insufficient`；瀏覽器端加 `discover_requested` 與 `login_resumed`。管理後台漏斗改成 `discover_requested → trip_created → offer_attached → outbound_click`，五語系文案跟著改。
...
`offer_attached` 的機票那一半（`from-offer`）在 #249 進 main 之後補齊，漏斗第三步完整。

...
狀態（2026-09-15）：**36 篇原稿、內容包、72 張配圖與練習素材已製作，網站整合及本機檢查已完成；33 篇仍需真實帳號、模型或裝置驗收，尚未整套發布。** 規劃與初次來源查證日為 2026-09-14，各篇另記實際查證日。查看 [本機候選包](release/README.md)、[逐篇待驗成果](release/ACCEPTANCE.md) 與 [Next 頁面驗證](platform/next-integration/README.md)。
...
2. 提供可下載且有授權說明的練習包：原始資料、提示詞或程式、預期結果、檢查清單、失敗案例。現有六批素材在 content/ 各主題中，作者紀錄保存檔案雜湊與本機驗證；課綱的成果要求仍須對照真實操作驗收。
...
- `tools/gemini-series.mjs` 已支援基本／深入兩種明確凍結清單，驗證數量、唯一 slug、引用及先修無循環。候選資料不能單靠數量正確就視為可發布；仍須比對完整內容與驗收紀錄。
...
以下保留原製作波次與任務分工。六個作者批次及平台整合已有本機交付，剩餘真實操作與發布由原任務持續追蹤；下表不是分批對外發布的紀錄。

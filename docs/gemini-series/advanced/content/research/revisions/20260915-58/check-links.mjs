...
const slugs = series.articles.filter(a => [58].includes(a.number)).map(a => a.slug);
const references = series.articles.filter(a => [57, 59, 60].includes(a.number)).map(a => a.slug);
...
console.log('Lesson 58 revised pack: catalogue, body length, assets and internal links passed.');

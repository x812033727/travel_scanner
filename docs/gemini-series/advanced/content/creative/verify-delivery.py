...
for source in sorted((HERE / "examples").glob("*.zip")):
    with zipfile.ZipFile(source) as archive:
        ...
        base = HERE if source.name == "creative-verification.zip" else HERE / "examples"
        for name in names:
...
with tempfile.TemporaryDirectory(prefix="gemini-creative-delivery-") as temporary:
    with zipfile.ZipFile(HERE / "examples/creative-verification.zip") as archive:

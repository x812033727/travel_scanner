...
for source in sorted((HERE / "examples").glob("*.zip")):
    with zipfile.ZipFile(source) as archive:
        ...
        base = HERE if source.name == "api-verification.zip" else HERE / "examples"
        for name in names:
...
with tempfile.TemporaryDirectory(prefix="gemini-api-delivery-") as temporary:
    with zipfile.ZipFile(HERE / "examples/api-verification.zip") as archive:

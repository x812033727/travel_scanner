...
const slugs = series.articles.filter(a => [52, 53, 54].includes(a.number)).map(a => a.slug);
const references = series.articles.filter(a => [51, 55].includes(a.number)).map(a => a.slug);
...
console.log('Three revised packs: catalogue, body length, assets and internal links passed.');

...
for source in sorted((HERE / "examples").glob("*.zip")):
    with zipfile.ZipFile(source) as archive:
        ...
        base = HERE if source.name == "work-verification.zip" else HERE / "examples"
        for name in names:
...
with tempfile.TemporaryDirectory(prefix="gemini-work-delivery-") as temporary:
    with zipfile.ZipFile(HERE / "examples/work-verification.zip") as archive:
...
receipt = {"checkedOn": "2026-09-14", "packages": packages, "extractedFixtureTests": tests, "modelCalls": 0, "sparkSchedulesCreated": 0, "publication": "unpublished"}

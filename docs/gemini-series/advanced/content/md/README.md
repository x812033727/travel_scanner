...
每篇正文 2,282–2,954 字，皆有四階段操作、故障練習、三個 FAQ、正文互連與返回總目錄。六組封面及圖解為原創 SVG，另有 1600×900 JPEG 封面與 360px 預覽。SVG 是教學概念圖，沒有偽裝成官方 UI 截圖。
...
2. Windows 原版 CLI 與 PowerShell `gemini.cmd` 入口已在 Node 22.23.2 各通過十四步生命週期；後者另外核對檔案副本與啟用狀態。Node 24.13.0、24.19.0 仍重現非零異常；[追加紀錄](verification/native-followup.md) 與[原失敗紀錄](verification/native-cli-limitations.json) 都保留。這不是所有 Windows 版本已修復，也不含實體視窗或模型操作驗收。

...
本篇已在 Windows、Node 24.19.0 使用實際 CLI ExtensionManager 模組驗證生命週期。追加測試中，Node 22.23.2 經原版 CLI 入口與 PowerShell 的 gemini.cmd 入口，各完成十四步安裝、更新及回退，所有結束碼為零。輸出重導到紀錄檔，沒有操作視窗截圖或模型對話；Node 24.13.0 與 24.19.0 仍重現 libuv 異常。
...
追加驗證保留原失敗紀錄，使用同一 CLI 0.59.0 與三個 Node 版本比較。24.13.0 在安裝後失敗，24.19.0 在工作區停用後失敗；22.23.2 的十四步命令通過。PowerShell 入口另驗十四步，還核對安裝檔案逐位元組相同、工作區停用為 false、啟用為 true，以及回退後完整來源為 1.0.0。這是特定組合的通過紀錄，不表示所有 Node 22 或 Windows 環境都不會崩潰。

若你的環境重現異常，可在獨立練習環境比對上述已驗證版本，先跑 node --version 與 gemini --version，再循序檢查結束碼、清單和副本。更新時若出現套件或技能確認，閱讀變更後回答；不能把等待確認誤判成安裝失敗。下載包的追加驗證說明提供可重做命令，原先的 Node 24 異常仍保留為已知限制。

> 本篇已驗證技能解析、三種腳本結果、管理模組，以及 Windows／Node 22.23.2 的 CLI 和 PowerShell 生命週期。Node 24 的異常尚未修復，模型觸發與回應品質仍未測；不可用本機安裝成功替代雲端驗收。

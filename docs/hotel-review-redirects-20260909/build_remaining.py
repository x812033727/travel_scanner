"""Carry forward only unchanged pending rows, retaining original evidence ages."""
...
def build():
    old = read(HERE.parent / "hotel-review-remaining-20260909/remaining-items.json")
    ...
    items = [{**prior[key], "evidence_age_note": (
        ...
        "not freshly re-reviewed or re-audited in the redirects batch."
    )} for key in sorted(pending)]
    options = [rows[key] for key in pending if key[0] == "option"]
    return {
        "as_of": final["captured_at"],
        "description": (
            "Only three new Agoda identities were reviewed in this batch. Remaining "
            "pending rows retain prior evidence; not a claim of full approval."
        ),

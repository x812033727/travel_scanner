...
SPEC = importlib.util.spec_from_file_location(
    "hotel_remaining_ops", REPO / "ops/hotel_review_remaining_20260909.py"
)
...
def option_entry(approve=False):
    key = next(
        ...
        if k[0] == "option"
        and r["discovery_status"] == "found"
        and r["url"]
        and r["product_id"] not in ops.OPERATING_DATE_HOLDS
    )
...
def test_pinned_baseline_and_subset_not_blanket_reaudit():
    assert len(ROWS) == 420 and len(PENDING) == 125
    assert sum(r["status"] == "approved" for r in ROWS.values()) == 295
    ...
    assert len(draft["decisions"]) == 125
...
def install_session(monkeypatch, data, existing_entries=()):
    ...
    class Session:
        async def __aenter__(self):
            self.saved_objects = {
                key: copy.deepcopy(ops.record(row)) for key, row in objects.items()
            }
            self.saved_added = len(added)
            return self

        async def __aexit__(self, *args):
            if args[0] is not None:
                for key, values in self.saved_objects.items():
                    for field, value in values.items():
                        setattr(objects[key], field, copy.deepcopy(value))
                del added[self.saved_added :]
            return False
...
def null_slot_entry():
    key = next(
        ...
        for k, r in PENDING.items()
        if k[0] == "option"
        and r["provider"] == "agoda"
        and r["url"] is None
        and r["product_id"] not in ops.OPERATING_DATE_HOLDS
    )
    entry = entry_for(*key, approve=True)
    url = "https://www.agoda.com/offline-exact-hotel-fixture/hotel/busan-kr.html"
    entry.update(option_patch={"url": url, "evidence_url": url}, source_urls=[url])
    return entry


def test_null_slot_patch_is_existing_identity_not_creation_or_quote_enablement():
    entry = null_slot_entry()
    row = ROWS[("option", entry["id"])]
    ops.validate_manifest(manifest(entry), PENDING)
    payload = ops.option_edit_payload(row, entry)
    assert payload.version == row["version"] > 0
    assert payload.provider == row["provider"] and payload.property_id is None
    assert payload.discovery_status == "found" and payload.url == entry["option_patch"]["url"]


@pytest.mark.parametrize(
    "field",
    ["property_id", "provider", "id", "product_id", "status", "discovery_status", "version"],
)
def test_null_slot_patch_rejects_unscoped_fields(field):
    entry = null_slot_entry()
    entry["option_patch"][field] = "unreviewed value"
    with pytest.raises(ValueError, match="exactly url"):
        ops.validate_manifest(manifest(entry), PENDING)
...
@pytest.mark.parametrize(
    ...
    [
        lambda e: e.update(method="primary_web", browser_verified=False),
        lambda e: e.update(browser_verified=False),
        lambda e: e.update(source_urls=["https://example.com/not-the-new-url"]),
        lambda e: e["option_patch"].update(url="https://www.agoda.com/"),
        lambda e: e["option_patch"].update(url="http://localhost/internal"),
        lambda e: e["option_patch"].update(evidence_url=""),
        lambda e: e.update(identity_note=""),
        lambda e: e.update(decision="hold"),
    ],
)
def test_new_slot_requires_complete_iab_evidence_and_safe_exact_page(mutation):
    entry = null_slot_entry()
    mutation(entry)
    with pytest.raises(ValueError):
        ops.validate_manifest(manifest(entry), PENDING)
...
@pytest.mark.parametrize(
    "field,value",
    [
        ("url", "https://www.agoda.com/existing-hotel/hotel/busan-kr.html"),
        ("evidence_url", "https://example.com/prior-evidence"),
...
def test_fill_cannot_replace_an_existing_identity_or_approved_slot(field, value):
    entry = null_slot_entry()
    row = copy.deepcopy(ROWS[("option", entry["id"])])
    row[field] = value
    with pytest.raises(ValueError, match="pending empty"):
        ops.option_edit_payload(row, entry)
...
async def test_null_slot_edit_review_atomicity_and_rollback(monkeypatch, failure):
    entry = null_slot_entry()
    data = manifest(entry)
    calls, objects, _, added = install_session(monkeypatch, data)
    row = objects[("option", entry["id"])]
    original = copy.deepcopy(ops.normalized(ops.record(row)))
    normal_actions = []

    async def edit(*, product_id, payload, user, session):
        assert isinstance(session, ops.DeferredCommit)
        assert payload.version == row.version == entry["version"]
        assert str(product_id) == original["product_id"] and payload.provider == row.provider
        for key, value in payload.model_dump(exclude={"version"}).items():
            setattr(row, key, value)
        row.status, row.version = "pending", row.version + 1
        row.health_status, row.checked_at, row.verified_at = "unchecked", None, None
        normal_actions.append("edit")
        await session.commit()
        assert calls["commit"] == 0
        if failure == "service_offer_mismatch":
            raise ops.AppError(409, failure, "Offline duplicate identity fixture")
        return {"id": uuid4() if failure == "wrong_id" else row.id}

    async def review(*, product_id, option_id, payload, user, session):
        assert (
            str(option_id) == entry["id"] and payload.version == row.version == entry["version"] + 1
        )
        normal_actions.append("review")
        if failure == "programming":
            raise RuntimeError("Offline unknown failure")
        if failure in {"service_link_unavailable", "service_version_conflict"}:
            raise ops.AppError(409, failure, "Offline review guard fixture")
        row.status, row.version = "approved", row.version + 1
        row.identity_note, row.health_status = payload.identity_note, "unconfirmed"
        row.checked_at, row.verified_at = stamp(), stamp()
        await session.commit()
    ...
    monkeypatch.setattr(ops.admin, "edit_hotel_option", edit)
    monkeypatch.setattr(ops.admin, "review_hotel_option", review)
    args = SimpleNamespace(mode="apply", manifest="memory")
    if failure in {"service_version_conflict", "programming", "wrong_id"}:
        with pytest.raises((RuntimeError, ops.AppError)):
            await ops.review_all(args, BASE, ROWS, PENDING)
        assert calls["commit"] == 0 and not added
        assert ops.normalized(ops.record(row)) == original
    else:
        await ops.review_all(args, BASE, ROWS, PENDING)
        assert calls["commit"] == 1 and len(added) == 1
        meta = added[0].metadata_json
        if failure:
            assert meta["outcome"] == "hold" and meta["guard_code"] == failure
            assert meta["before_hash"] == meta["after_hash"] == entry["before_hash"]
            assert ops.normalized(ops.record(row)) == original
        else:
            assert normal_actions == ["edit", "review"]
            assert row.version == entry["version"] + 2 and row.url == entry["option_patch"]["url"]
            assert row.property_id is None and row.status == "approved"
            assert meta["option_patch"] == entry["option_patch"] and meta["outcome"] == "approved"


@pytest.mark.asyncio
async def test_new_slot_approved_receipt_replays_without_reedit_or_new_url_write(monkeypatch):
    entry = null_slot_entry()
    data = manifest(entry)
    old_evidence(monkeypatch, data)
    calls, objects, receipts, added = install_session(monkeypatch, data, [entry])
    row = objects[("option", entry["id"])]
    row.url, row.evidence_url = entry["option_patch"]["url"], entry["option_patch"]["evidence_url"]
    row.identity_note, row.discovery_status = entry["identity_note"], "found"
    row.status, row.version = "approved", row.version + 2
    receipts[0].metadata_json.update(
        outcome="approved",
        after_version=row.version,
        after_hash=ops.digest(ops.normalized(ops.record(row))),

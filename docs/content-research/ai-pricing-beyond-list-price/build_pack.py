"""Builds pack.json for ai-pricing-beyond-list-price.
...
blocks = [
    ...
    {"type": "paragraph", "text": "Anthropic 是少數把這件事寫在定價頁上的：Claude 4.7 以後的模型換了新的 tokenizer，同一段文字大約會多產生 30% 的 token，官網並註明實際增幅依內容而定，而 Claude Sonnet 4.6 以前的模型用的是舊的。換句話說，同一家的新舊模型之間，單價相同也不代表帳單相同。"},
    {"type": "paragraph", "text": "這件事的實際後果是：單價便宜一成，如果 token 數多三成，總花費反而是貴的。所以拿兩家比價的時候，真正能比的是「同一份文件各自數出來幾個 token、乘上各自的單價」，不是把兩個單價並排看。各家官網都有 token 計數的端點或工具，拿你自己真正會送出去的內容去數一次，比看價目表準得多。"},
    {"type": "paragraph", "text": "另外兩家的 tokenizer 差異，官網當天沒有可以直接引用的對照說明，所以這篇沒有替它們填數字。要比的話只能自己拿同一份文件分別去數。"},
    ...
    {"type": "rich_paragraph", "inlines": [
        txt("各家的基礎單價、上下文視窗與權重授權排在一起的總表在"),
        art("各家 AI 模型總表", "ai-model-comparison-table-2026"),
        txt("；只想看價目表與免費層條款的話，"),
        art("API 價格比較", "ai-api-pricing-comparison-2026"),
        txt(" 那篇寫得更細。快取本身的原理另有"),
        art("提示詞快取", "ai-term-prompt-caching"),
        txt("一篇。"),
    ]},

    {"type": "heading", "level": 2, "text": "怎麼用這三件事重估一次"},
    {"type": "paragraph", "text": "這三欄不用全部算過，照下面的順序看一遍，通常兩三分鐘就知道自己的估算偏在哪裡。"},
    ...
    {"type": "rich_paragraph", "inlines": [
        txt("怎麼實際數自己的 token、有哪些官方計數工具，站上有"),
        art("token 計算與費用估算", "ai-token-cost-estimation"),
        txt("；"),
...
print("blocks:", len(blocks), "| h2:", sum(1 for b in blocks if b["type"] == "heading"),
      "| tables:", sum(1 for b in blocks if b["type"] == "table"),
      "| sources:", len(sources))
print("title:", len(pack["locales"]["zh-TW"]["title"]), "chars | description:", len(pack["locales"]["zh-TW"]["description"]), "chars")
print("body:", body, "chars | document json:", len(json.dumps(pack["locales"]["zh-TW"], ensure_ascii=False)), "chars")

...
下載[練習專案起始版](article:claude-code-practice-project-setup)，在獨立資料夾解壓縮，確認 npm test 通過。若想對照結果，可另下載[參考完成版](https://mokaair.com/tutorials/claude-code/complete.zip)，放在另一個資料夾；完成版還包含保存功能，本篇只做篩選，不必先完成前面所有文章。

## 先寫出使用者行為

建立兩筆待辦，將其中一筆完成。按「未完成」應只看到尚未完成的項目；按「已完成」看到完成項目；切回「全部」仍有兩筆。這個例子明確說明篩選不能把不符合的資料刪掉，也不能因顯示順序改變而操作錯誤項目。

```text Claude Code 對話框：規劃本次功能
...
```javascript 加入 model.js：不要重複加入已存在的同名函式
...
```text Claude Code 對話框：整合 app.js
...
```javascript 寫入 tests/filter.test.mjs
...
> 先讓功能通過明確驗收，再考慮額外美化與保存。一次只增加一個可驗證行為，較容易知道錯誤從哪一步出現。

小練習是加入空清單情境，確認某個篩選沒有結果時仍可切回其他模式。完成判準是測試通過、三種篩選與勾選刪除正常、資料不丟失且手機可讀。最後請 Claude 整理實際差異與未驗證部分，接著用[Git 審查](article:claude-code-git-review-pr)準備交付。

...
- **表一（4 欄）：路線｜飛機｜火車｜備註**，5 列：
  ...
  4. 順化⇄峴港｜越南航空網站上沒有這段的航線頁，以航空公司官網為準｜SE1 順化 10:30 發、13:14 到峴港，約 2 小時半到 2 小時 45 分（依班次不同）｜票價要回 dsvn.vn 訂票流程查（見 H2-3 的警告）
...
- **H3「SE1 到 SE8：南下四班、北上四班」**：用 list 寫，時刻逐字照官方查詢結果，一行一個車次：
  ...
  - 南下 SE3：河內 19:20 → 順化 07:50 到（07:55 開）→ 峴港 10:28 → 西貢 05:30（第三天）
  - 南下 SE5：河內 08:00 → 順化 21:35 到（21:40 開）→ 峴港 00:21 → 西貢 18:20（隔天）
  - 南下 SE7：河內 06:00 → 順化 19:38 到（19:43 開）→ 峴港 22:15 → 西貢 17:00（隔天）
  - 北上 SE2：西貢 20:35 → 峴港 13:06 到（13:21 開）→ 順化 15:49 → 河內 05:42（第三天）
  - 北上 SE4：西貢 19:20 → 峴港 12:13 到（12:28 開）→ 順化 15:05 → 河內 04:36（第三天）
  - 北上 SE6：峴港 01:46 到（02:01 開）→ 順化 04:44 → 河內 19:14
  - 北上 SE8：西貢 06:00 → 峴港 23:21 到（23:41 開）→ 順化 02:12 → 河內 16:20（隔天）
...
兩句，各一個 article inline：一句講從峴港去順化怎麼安排一天（連本批第 17 篇），一句講河內出發的下龍灣行程（連本批第 16 篇）。連結前的句子要寫成拿掉連結後仍讀得通，例如「順化在峴港北邊，SE 車次約 2 小時半到 2 小時 45 分」「下龍灣在河內東邊，怎麼去、船怎麼挑另有一篇」。
...
- 鐵路縱線上的區段標籤（`#5C6B6B`，15 px）：河內–順化 688 公里、順化–峴港（不標里程，標「約 2 小時半到 2 小時 45 分」）、河內–峴港 791 公里、河內–西貢 1,726 公里。
...
- 左上角 `y="52"` 30 px 粗體中文標題「越南南北：飛機與統一鐵路」、`y="84"` 18 px 英文副標「Vietnam domestic: flights and the Reunification Express」；右下角 `x="1540" y="870" text-anchor="end" font-size="15" fill="#5C6B6B"` 放 `© Mokaair 製圖 2026`；底部一行 15 px 小字「時刻與票價 2026 年 9 月查證，以當日查詢為準」，同一行（或緊接的下一行）15 px 小字再加「示意圖，方向與距離非比例」。
- **圖上允許出現的數字，全部都要在正文出現**：688、791、1,726、1 小時 20 分、1 小時 15 分、2 小時 10 分、20、3、26、32、5、10、21:45、13:14、06:30、2 小時半到 2 小時 45 分、2026。
...
(19) **火車的「幾小時」是自己算的。** 「約 15 個半小時」「約 12 個半小時」「約 2 小時半到 2 小時 45 分」是從官方的發車與到站時刻相減得來的概數（順化⇄峴港南下 SE1 是 2 小時 44 分、北上 SE2 是 2 小時 28 分、SE4 是 2 小時 37 分，所以寫成區間），只能寫概數，而且同一句要寫出發與到站時刻讓讀者自己對；不要寫「15 小時 29 分」這種精確值，也不要寫平均時速。
...
(22) **中途站有到站與發車兩個時刻。** dsvn.vn 的表上「Giờ đến／Giờ đi」是兩欄：順化、峴港這些中途站，到站與發車差 5 到 20 分。本篇 H2-3 的 list 一律寫成「到站（發車）」雙時刻，和 SE1 那一行的寫法一致；第 17 篇（順化一日遊）用的是**發車時刻**，兩篇的發車時刻必須一模一樣（SE3 順化 07:55、SE5 順化 21:40、SE7 順化 19:43、SE2 峴港 13:21、SE4 峴港 12:28、SE6 峴港 02:01、SE8 峴港 23:41）。SE8 峴港的到站與發車差 20 分（其餘差 5 或 15 分），撰稿當天要特別重查這一班，確認不是其中一篇抄錯。
...
- **本批互連的一致性**：第 16 篇（下龍灣）與第 17 篇（順化）的交通段會連本篇。三篇共用的說法必須一字不差：SE 車次的**發車時刻**（本篇列「到站（發車）」雙時刻，第 17 篇列發車時刻，括號裡的發車時刻要和第 17 篇一模一樣）、「順化到峴港約 2 小時半到 2 小時 45 分（依班次不同）」、「票價以 dsvn.vn 當日查詢為準」。任何一篇改時刻或改說法，同一個 PR 改其他篇。

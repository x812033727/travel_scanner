...
- 和本批第 18 篇 `vietnam-domestic-flights-train-guide`（撰稿中）：統一線的通則歸它——車廂等級、臥鋪、退換票、線上購票步驟、河內與西貢出發的票價、國內線航空。本篇只寫峴港到順化這一段的班次時刻與當天來回的判斷，其餘一句話連過去。**兩篇若都寫到 SE1、SE3、SE4、SE2 的時刻，必須是同一次查詢的同一組數字**；中途站有到站與發車兩個時刻，本篇一律用**發車時刻**（表格的「峴港 12:28」「順化 07:55」都是發車），第 18 篇列的是「到站（發車）」雙時刻，兩篇括號內外的發車時刻要一模一樣。第 18 篇先上線就打開它的規格與 `pack.json` 對過再寫。
...
表後依序寫：
- 這一段 103 公里，車程約 2 小時半到 2 小時 45 分（依班次不同：SE2 是 2 小時 28 分、SE4 是 2 小時 37 分、南下 SE1 是 2 小時 44 分；正文寫這個區間就夠，不要逐班列分鐘）。四班北上車只有 SE4 與 SE2 是白天，另外兩班在深夜與凌晨；南下回峴港的白天班次是 SE3 07:55 與 SE1 10:30，晚上是 SE7 19:43 與 SE5 21:40。
...
左半邊畫「峴港到順化」：
- 右下角是峴港 Đà Nẵng，左上角是順化 Huế，兩點之間畫兩條線：**實線是鐵路**（標「統一線 103 公里・約 2 小時半到 2 小時 45 分」，線上標北上「SE4 12:28／SE2 13:21」、南下「SE3 07:55／SE1 10:30」），**虛線是公路**（標「包車約 2 小時」）。
...
圖上會出現的數字全部要在正文：103、2 小時半到 2 小時 45 分、12:28、13:21、07:55、10:30、2 小時、70,000、200,000、150,000、420,000、530,000。**不畫**：530,000 以外的聯票組合價、600,000 全區聯票、兒童票、巴士與包車價格、陵墓公里數、開放時間、SE6／SE8／SE5／SE7 的時刻（那四班在表裡就好，圖上放不下也容易亂）。越南文只用官方頁讀到的寫法：Đà Nẵng、Huế、Hải Vân Quan、Đại Nội、Chợ Đông Ba、Chùa Thiên Mụ、Lăng Tự Đức、Lăng Khải Định、Lăng Minh Mạng、Sông Hương、Ga Huế。
...
（19）數字四處一致（正文、summary、表格、FAQ、圖解）：200,000／40,000、150,000／30,000、100,000、50,000、70,000、420,000／80,000、530,000／100,000、600,000／120,000、240,000、200,000（2 點聯票）、350,000、500,000、650,000、103、2 小時半到 2 小時 45 分、2 小時、12:28、13:21、07:55、10:30、19:43、15:05、143 年。改一處要全改，並檢查第 18 篇有沒有同一個數字。
...
- 與第 18 篇的口徑：統一線時刻若兩篇都寫，必須同一次查詢的同一組數字（本篇寫發車時刻，第 18 篇寫「到站（發車）」雙時刻，發車時刻要一致），順化⇄峴港的車程兩篇都寫「約 2 小時半到 2 小時 45 分（依班次不同）」；第 18 篇寫河內與西貢出發的票價、車廂等級、退換票，本篇不重複。第 18 篇上線前後各對一次；它若沒上線，拿掉本篇 H2-1 的連結與 `related` 裡的它。

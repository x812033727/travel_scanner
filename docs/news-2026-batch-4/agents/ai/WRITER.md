...
2. `ROOT\docs\news-2026-batch-4\ai.md` — AI 垂直的界線：這一批只補漏，**動筆前先讀它列的既有 38 篇標題，不要重寫別人寫過的事**；廠商的能力宣稱一律歸因（「OpenAI 表示」），本站沒有實測；不寫訂閱或購買建議；**AI 篇不帶 `finance` 主題、沒有投資免責 callout**（B8 金融服務那篇與 B2 募資、B6 S-1 也一樣：不可寫成投資題材，不寫估值判斷、不預測上市，用一般 callout 明講本文不是投資建議）。
3. `ROOT\docs\news-2026-batch-4\corrections-ai.md` — **對你有拘束力**。讀你的 slug 那一段（`## <slug>` 到下一個 `## `），以及檔尾「跨篇通則」。用 Grep 找行號再用 Read 的 offset／limit 讀，不要整份讀。
...
6. JSON 形狀的範本（已上線的一篇）：內容包 `ROOT\apps\api\app\guides\content\crypto-news-genius-act-occ-20260302.json` 的 `locales["zh-TW"]`，研究紀錄 `ROOT\docs\crypto-news-2026\research\crypto-news-genius-act-occ-20260302.json`。
   **和範本不同的三件事**：AI 篇只有**一個** callout（範本有兩個，第二個是幣圈免責，你不要寫）；範本結尾的兩個連結已被工具改寫成 `rich_paragraph`，你要寫的是下面第 1 節的純 `link` 區塊；`topics` 不同。
...
- 工作區研究紀錄：`ROOT\docs\ai-news-2026-09-late\research\<slug>.json`（套用修正清單**之後**的版本，工具讀的是這一份；`research` 目錄不存在就先建）
...
```json
{"slug":"<slug>","kind":"life","destination_id":null,"topics":["ai","software","ai-news"],
...
`topics`：預設 `["ai","software","ai-news"]`；公司層級的新聞（募資、S-1、併購、治理框架）用 `["ai","ai-news"]`；主體是晶片／硬體用 `["ai","gadgets","ai-news"]`。**不可以加 `finance`。**

`display_order`：openai-astral 149、openai-funding 150、gpt-55-instant 151、chatgpt-ads 152、frontier-governance 153、openai-s1 154、openai-broadcom-chip 155、chatgpt-financial-services 156、gpt-live-1-api 157、chatgpt-storage-scale 158、gemini-38-live 159、nvidia-hugging-face 160。
...
9. 兩個 `link`：
   - 第一個：`{"type":"link","text":"2026 年 AI 新聞總整理：1 月至 9 月的重點與生活應用","url":"https://mokaair.com/zh-TW/life/ai-news-2026-january-september-index"}`（text 逐字照抄）
...
- **只允許留下這兩種 FAIL**（索引與相關文章都還沒寫好）：`link target ai-news-2026-january-september-index.json does not exist yet`，以及第二個連結的 `link target … does not exist yet` 或 `link text must be the title of …`。其他 FAIL 都要修到消失。不可以為了過檢查而刪掉查證過的條件或限制。
...
1. **來源可能被擋**：`openai.com/index/*` 與 `help.openai.com` 對 curl 常回 403。研究紀錄已經找過官方替代管道（官方 RSS、`cdn.openai.com`、`platform.openai.com` 文件、對方公司的新聞稿、EDGAR 等），照研究紀錄與修正清單的 `source_list_fix` 用；你今天仍要用 `curl -sL -A "Mokaair-editorial"` 重抓 `sources[]` 每一條並讀到正文。**讀不到正文的來源不能撐任何句子**——那句話就不寫，記進 `unverified_or_excluded`，並在回報第 5 點寫明；不可以改用整合站、媒體或百科補。研究紀錄 `sourcing_verdict` 是 `partial` 的三篇（chatgpt-ads、gpt-55-instant、openai-s1）尤其要守：寫得短沒關係（下限 1,800 字），寫來源撐不住的不行。
2. **索引的標題會改**：第一個 link 的 text 照上面給的新標題逐字抄，`check_article.py` 會因此留下 `link text must be the title of ai-news-2026-january-september-index` 這條 FAIL，這是允許的（協調者之後用 `update_index.py` 原地改索引標題）。
3. **查核日**：正文第二段與各處的查核日寫你今天實際重抓來源的日期（執行 `date +%F` 看；台北時間），`checked_on` 同一天。修正清單寫的「一律 2026-09-16」是前期研究的日期，依「BRIEF > 修正清單」以實際查證當天為準。

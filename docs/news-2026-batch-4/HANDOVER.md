# 交接：新聞批次 4（2026-09-18：幣圈已發布，科技十三篇加索引待站主驗收）
...
分支 `claude/news-batch-4-2-tech`，**還沒合併、還沒匯入發布——兩件事都要等站主指示**（見第 1b 節）。
...
分支 `claude/news-batch-4-2-tech`。十四個內容包都是五語、五語圖檔齊全、都經過**兩輪**獨立查核與逐語審稿；
十三篇都過 `check_article.py --full --assets`，索引五語過 schema 與 lint，`pack_cli lint --kind life` 0 error。
**還沒合併、還沒匯入發布。**「查核」欄是第一輪核對的主張數／改動處數，與第二輪再改的處數。
...
- **發布當天要重開的活頁面**：歐盟 CRA 那篇的法條來源換成了 EUR-Lex 的 ELI 網址（原本的 CELLAR 網址在中文瀏覽器會回純文字錯誤頁），
  但 2026-09-17 EUR-Lex 部分停擺，**沒有人親眼確認過那一頁打得開**；ENISA 常見問答是活文件（第 9 題當天標 `[UPDATED]`）；
...
**科技（4.2）**：內容做完了，剩站主驗收、合併 PR、部署、`guides-import --slug` ×14 同一次匯入（`related` 才會在第二輪套上；
`tech-news-nvidia-vera-rubin-20260915` 的延伸閱讀另外連到已發布的 `ai-news-nvidia-rubin-20260105`）。照 2.3 的做法，並先重開第 1b 節列的活頁面。

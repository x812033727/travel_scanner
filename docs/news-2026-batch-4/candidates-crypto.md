...
## ⚠️ 兩條台灣的官方管道，2026-09-16 查核才確認可用

- **金管會新聞列表有 server-side 的關鍵字與日期區間搜尋，不需要 JavaScript。**
  先前的研究把它寫成「外包給 Google CSE、分頁靠 JS，不能當一手管道」而放棄，
  結果漏讀了兩篇直接相關的新聞稿。可用的做法是對
  `https://www.fsc.gov.tw/ch/home.jsp` 送表單參數
  `id=96`、`parentpath=0`、`mcustomize=news_list.jsp`、`dtable=News`、
  `aplistdn=ou=news,ou=multisite,ou=chinese,ou=ap_root,o=fsc,c=tw`、
  `keyword=<關鍵字>`、`page=1`、`pagesize=100`，回 200 且是 server-rendered 的結果表格，
  也吃 `qptdate`／`qdldate` 日期界限。限制是**只搜標題、不搜全文**，但它是一手的。
- **行政院公報資訊網（`gazette.nat.gov.tw`）有可用的全文查詢端點**
  （`advancedSearchResult.do?action=doQuery&keywords=<urlencoded>&fields=text`）。
  台灣主管機關多半沒有 feed，**這等於它們版本的 Federal Register**。
...
- **⚠️ 2026-09-16 查核更正了三處法條層級的寫法，撰稿照下面寫，不要照上面的新聞稿摘要。**
  - **不是兩張許可。** 第 34 條的機制是**向主管機關（金管會）申請許可，
    金管會為許可前應會商中央銀行同意**。寫成「須經中央銀行與金管會許可」會被讀成兩張。
  - **不是準備資產 100% 信託。** 新聞稿的「維持十足準備資產及交付信託」是正確引述，
    但第 36 條第 1 項比它窄：準備資產須十足、儲存於境內金融機構、與自有財產分別獨立，
    且「**除依第二項規定繳存之準備金外**，應全數信託」。
  - **「得延長 3 個月」只掛在 21 個月上。** 第 55 條第 2 項寫的是「前項**取得許可證照期限**」，
    不是 12 與 21 都能延。
- **本篇的骨幹三件事，候選清單原本漏了**：公布日 **2026-07-22**
  （總統華總一經字第11500067161號令，全文 **56 條**）；**施行日期由行政院定之**；
  全國法規資料庫在 2026-09-16 顯示「最後生效日期：**未定**」。
- **全國法規資料庫的正確 pcode 是 `G0400163`**（用站內整合查詢 `ty=ONEBAR&kw=虛擬資產服務法`
  搜出來的，不是猜的）。`BRIEF.md` 記的那個被杜撰的 `G0380347` 不是本法。
- **新聞稿明載**：七種服務商（交換商、交易平台商、移轉商、保管商、承銷商、借貸商及其他）；
  境內發行穩定幣須經許可，並「維持十足準備資產及交付信託」；

...
try {
  for (const viewport of ['desktop', 'mobile']) {
    ...
    for (const entry of manifest) {
      ...
      report.pages.push({slug:item.slug,locale,viewport,status:response.status(),full_text:true,images:images.length,table:true,sources:doc.sources.length,internal_links:doc.blocks.filter(b=>b.type==='link').length,metadata:true,screenshot:`browser/${viewport}-${locale}-${item.slug}.png`});
    ...
    // The first page may be paginated; the public API inventory separately covers all new articles.
  ...
  for(const locale of locales) {
  let cursor=null;const articles=[];
  do {
  ...
  report.sitemap_locale_articles=110;
  assert.equal(report.pages.length,220);
  await api.close();

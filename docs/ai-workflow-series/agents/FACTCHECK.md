...
repo 根目錄與暫存目錄由指派給實際路徑（以下 ROOT 與 SCRATCH；路徑一律用正斜線寫）。
...
4. 範例報告（格式與嚴格度照它；那是新聞篇，日期與事件日的部分不適用）：`ROOT/docs/news-2026-batch-4/factcheck-draft/ai-news-chatgpt-storage-scale-20260911.md`。

## 1. 怎麼查

- 把 `sources[]` 每一條**今天重抓一次並讀 body**（`curl -sL -A "Mokaair-editorial"`；GitHub README 用 `raw.githubusercontent.com`；PDF 用有 `pypdf` 的 Python 抽字）。HTTP 200 不等於拿到文件：JS 渲染的文件站可能回空殼，先看 bytes 與是否含正文關鍵字；讀不到正文的來源不能撐任何句子。
...
- 特別找這幾種：
  ...
  8. **比對 `verbatim_quote` 的方法要固定**：把來源 HTML 的標籤刪成**空字串**（不是換成空白），再把連續空白正規化成一個空格、HTML 實體還原後做子字串搜尋；`<code>`、`<strong>` 這類行內標籤若換成空白會在字邊界造假空白，第一輪與第二輪曾因此得出相反結論。Markdown 來源另還原反引號、`>` 與硬斷行。
...
`VENV_PYTHON` 是 API 專案 venv 的直譯器，指派會給實際路徑（Windows `./.venv/Scripts/python.exe`，Linux／macOS `./.venv/bin/python3`）。

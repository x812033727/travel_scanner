...
test.beforeEach(async ({ page }) => {
  ...
  await page.route("**/api/travel/hotspots/**", (route) => {
    ...
    if (url.pathname.endsWith(`/${hotspotId}/place`)) return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ hotspot_id: hotspotId, hotspot_name: "Senso-ji", status: "ready", google_maps_url: "https://www.google.com/maps/place/?q=place_id:test", official_website_url: "https://www.senso-ji.jp/", official_website_verified: true, has_details: true, updated_at: "2026-09-01T00:00:00Z", address: "2-3-1 Asakusa, Taito City, Tokyo", plus_code: { global_code: "8Q7XPR6F+82", compound_code: "PR6F+82 Taito City" }, coordinates: { latitude: 35.714765, longitude: 139.796655, source: "google_places" }, opening_hours: { weekday_descriptions: ["Monday: 6:00 AM–5:00 PM"] }, data_locale: "en", fetched_at: "2026-09-01T00:00:00Z", expires_at: "2026-10-01T00:00:00Z", attribution: { provider: "Google Maps", provider_url: "https://maps.google.com", third_party: [] } }) });
...
for (const width of [320, 390]) {
  test(`mobile guide sheet is app-like at ${width}px`, async ({ page }) => {
    ...
    await expect(dialog.getByText("PR6F+82 Taito City")).toBeVisible();

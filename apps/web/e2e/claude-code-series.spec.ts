...
const evidence = fileURLToPath(new URL("../../../docs/claude-code-series/evidence/", import.meta.url));
...
test("directory server-renders all 60 published links without JavaScript", async ({ browser }) => {
...
test("search aliases, combined filters, URL reload and browser back", async ({ page }) => {
  ...
  for (const [term, slug] of [["MD", "claude-md-guide"], ["手機", "mobile-ios"], ["權限", "permissions-plan-mode"], ["/compact", "context-compact-clear"], ["MCP", "mcp-servers"]]) {
  ...
  await expect(directory.locator("ol > li > a")).toHaveCount(60);
...
test("all 61 pages render and every internal reference has a valid target", async ({ page }) => {
  ...
  for (const [slug, pack] of packs) {
    ...
    if (index < 59) await expect(navigation.getByRole("link", { name: /^下一篇：/ })).toHaveAttribute("href", `/zh-TW/life/${catalogue.entries[index + 1].slug}`);
...
test("360 and 390px articles retain TOC, readable tables and contained code", async ({ page }) => {
  for (const width of [360, 390]) {
    ...
    for (const suffix of ["claude-md-guide", "hooks-getting-started", "templates-cheatsheet"]) {
...
test("downloaded complete lab supports filtering, persistence, safe text and corrupted storage", async ({ page, context }) => {
  const lab = await startPracticePreview();
  try {
...
test("copy preserves raw input, indentation and text through the native clipboard", async ({ page, context }) => {
  ...
  for (const suffix of ["markdown-basics", "hooks-getting-started", "implement-feature"]) {

...
export default async function RegisterPage({ searchParams }: { searchParams: Promise<{ next?: string | string[] }> }) {
  ...
  return <><SiteHeader /><main className="mx-auto max-w-md px-5 py-14"><div className="rounded-[2rem] border border-[var(--line)] bg-white p-8"><p className="text-sm font-semibold text-[var(--teal)]">{t("registerBonus")}</p><h1 className="mt-2 text-3xl font-bold">{t("registerTitle")}</h1><p className="mt-3 text-sm leading-6 text-[var(--muted)]">{t("registerDescription")}</p><AuthForm mode="register" nextPath={nextPath} /><p className="mt-5 text-center text-sm text-[var(--muted)]">{t("hasAccount")} <Link className="text-[var(--teal)] underline" href={`/login?next=${encodeURIComponent(nextPath)}`}>{t("signInDirectly")}</Link></p></div></main></>;
}

...
export async function generateMetadata(
  ...
  return topicHubMetadata({ locale, section: "travel", topic, cursor: search.cursor, sort: search.sort });
}
...
export default async function TravelTopicHubPage(
  ...
  const [{ locale, topic }, search] = await Promise.all([params, searchParams]);
  return renderTopicHub({ locale, section: "travel", topic, cursor: search.cursor, sort: search.sort });
}

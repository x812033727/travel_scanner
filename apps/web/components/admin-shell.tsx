    ...
    <div className="admin-workspace">
      <header className="admin-topbar">
        ...
        <div className="admin-topbar-actions">
          ...
          <button ref={commandTrigger} type="button" onClick={() => setCommandOpen(true)} className="admin-command-trigger"><Search aria-hidden size={17} /><span>{copy.command}</span><kbd><Command aria-hidden size={11} />K</kbd></button>
          <div ref={accountMenu} className="relative">
            <button type="button" onClick={() => setAccountOpen((value) => !value)} aria-expanded={accountOpen} className="admin-account-trigger"><span className="admin-account-avatar"><UserRound aria-hidden size={17} /></span><span className="admin-account-email">{bootstrap.user?.email || user?.email}</span><ChevronDown aria-hidden size={15} /></button>

...
export function RouteTimelineLink({ edge, segment: saved, nextTitle, loading, stale, needsSetup, onClick }: {
  ...
  const status = needsSetup ? "pending" : edge?.status || (stale || segment?.status === "stale" ? "stale" : segment ? "ready" : "estimated");
  if (status === "pending") return <div className="planner-route-connector planner-route-pending flex min-h-11 items-center gap-2 px-2 text-xs text-[var(--muted)]"><Route size={14} aria-hidden /><span>{copy.pending}</span></div>;
  ...
  const minutes = segment?.duration_minutes ?? edge?.estimatedMinutes;
  const label = status === "stale" ? copy.stale
    : minutes == null ? copy.details
      : status === "estimated" ? copy.estimated.replace("{minutes}", String(minutes)) : t("minutes", { minutes });

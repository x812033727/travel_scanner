...
import { adminCatalogCopy, hotspotIdentityHref } from "@/lib/admin-catalog-copy";
...
export function AdminHotspotPlacesPanel() {
  ...
  async function saveEdit(event: FormEvent) {
    ...
    try {
      const result = await api<{ run: EnrichmentRun | null }>(`/admin/hotspots/${edit.hotspotId}/place-profile`, {
        method: "PATCH", body: JSON.stringify({ action: "save", official_website_url: edit.website || null, official_website_source_url: edit.websiteSource || null }),
      });
      if (result.run) setRun(result.run);
  ...
  return <section className="mt-10 border-t border-[var(--line)] pt-8">
    ...
    {edit && <form onSubmit={saveEdit} className="mt-4 rounded-2xl border border-[var(--teal)] bg-white p-4"><div className="flex items-center justify-between"><h3 className="font-bold">{t("hotspotPlacesPanel.editRow", { name: edit.name })}</h3><button type="button" onClick={() => setEdit(null)} className="min-h-11 px-3 text-sm">{t("hotspotPlacesPanel.cancel")}</button></div><div className="mt-3 grid gap-3 md:grid-cols-3"><div className="text-sm font-semibold"><span>Google Place ID</span><p className="mt-1 break-all font-mono font-normal">{edit.googlePlaceId || t("hotspotPlacesPanel.noPlaceId")}</p><p className="mt-2 text-xs font-normal text-[var(--muted)]">{copy.identityHint}</p><Link href={hotspotIdentityHref(edit.hotspotId)} className="mt-2 inline-flex min-h-11 items-center font-semibold text-[var(--teal)] underline">{copy.identityEditor}</Link></div><label className="text-sm font-semibold">{t("hotspotPlacesPanel.officialWebsite")}<input type="url" value={edit.website} onChange={(event) => setEdit({ ...edit, website: event.target.value })} className="mt-1 h-11 w-full rounded-xl border px-3 font-normal" /></label><label className="text-sm font-semibold">{t("hotspotPlacesPanel.websiteSourceLabel")}<input type="url" value={edit.websiteSource} onChange={(event) => setEdit({ ...edit, websiteSource: event.target.value })} className="mt-1 h-11 w-full rounded-xl border px-3 font-normal" /></label></div><button type="submit" disabled={loading} className="mt-4 min-h-11 rounded-xl bg-[var(--teal)] px-5 text-sm font-semibold text-white disabled:opacity-40">{t("hotspotPlacesPanel.save")}</button></form>}
  </section>;
}

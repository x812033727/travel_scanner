...
export function RouteMap({ items, segment }: { items: TripItem[]; segment?: RouteSegment }) {
  ...
  const src = canEmbed ? `https://www.google.com/maps/embed/v1/directions?key=${encodeURIComponent(browserKey)}&origin=${origin.latitude},${origin.longitude}&destination=${destination.latitude},${destination.longitude}&mode=transit&language=zh-TW` : undefined;
  return <section className="overflow-hidden rounded-[1.75rem] border border-[var(--line)] bg-white shadow-sm lg:sticky lg:top-5">
    ...
    {src ? <iframe title="行程路線地圖" src={src} className="h-[420px] w-full border-0" loading="lazy" referrerPolicy="no-referrer-when-downgrade" /> : <div className="grid min-h-[360px] place-items-center bg-[linear-gradient(135deg,#edf5f1,#f8f5ee)] p-8 text-center"><div><MapPin size={30} className="mx-auto text-[var(--teal)]" /><p className="mt-3 font-semibold">{segment ? "路線步驟已準備好" : "計算路線後可在這裡查看地圖"}</p><p className="mx-auto mt-2 max-w-xs text-sm leading-6 text-[var(--muted)]">{segment ? "設定受網域限制的瀏覽器地圖金鑰後，這裡會顯示互動路線；詳細搭車步驟仍可直接查看。" : "每個地點確認位置後，選擇「計算路線」。"}</p></div></div>}

...
export function RouteMap({ items, segment }: { items: TripItem[]; segment?: RouteSegment }) {
  ...
  const src = canEmbed ? `https://www.google.com/maps/embed/v1/directions?key=${encodeURIComponent(browserKey)}&origin=${encodeURIComponent(originValue)}&destination=${encodeURIComponent(destinationValue)}&mode=transit&language=zh-TW` : undefined;

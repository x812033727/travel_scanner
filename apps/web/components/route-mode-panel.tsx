...
export function RouteModePanel({
  ...
  const activeSegment = preview?.segment
  ...
  const directionsUrl = fromItem && toItem
  ...
  return <div className="route-panel-layout">
    <div className="route-panel-controls space-y-4">
      ...
      {resolvingLocations && <div className="route-preview-skeleton compact" aria-live="polite"><Loader2 size={22} className="animate-spin text-[var(--teal)]" /><strong>正在補齊兩端地點…</strong><span>以旅程目的地限制 Google 搜尋範圍</span></div>}
      ...
      {!resolvingLocations && !loadingMode && !activeSegment && !localError && !unresolvedItems.length && <div className="route-empty-state"><MapPin size={20} /><div><strong>尚未取得路線</strong><p>取得 Provider 驗證結果後，才會開放套用。</p></div></div>}
    ...
    {activeSegment && <div className="route-panel-detail min-w-0 space-y-4"><RouteMap items={items} segment={activeSegment} fromItemId={fromItemId} toItemId={toItemId} travelMode={mode} variant="drawer" /><RouteSegmentCard segment={activeSegment} selected defaultExpanded timezone={trip.timezone} /></div>}

    <div className="route-apply-bar"><div className="min-w-0"><span className="block text-xs text-[var(--muted)]">目前選擇</span><strong className="block truncate">{modes.find((item) => item.value === mode)?.label}{activeSegment ? ` · ${activeSegment.duration_minutes} 分鐘` : " · 尚未取得"}</strong></div><button type="button" onClick={() => preview ? void applyPreview() : void previewMode(mode)} disabled={applying || Boolean(loadingMode) || resolvingLocations || unresolvedItems.length > 0 || isApplied} className="flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-xl bg-[var(--teal)] px-5 font-bold text-white disabled:opacity-45">{applying ? <Loader2 size={17} className="animate-spin" /> : isApplied ? <Check size={17} /> : null}{isApplied ? "目前已套用" : preview ? "套用此路線" : loadingMode ? "取得中…" : "取得路線"}</button></div>
  </div>;
}

...
export function AccountList({ kind }: { kind: "trips" | "alerts" }) {
  ...
  return <div className="space-y-3">{items.map((item) => <article key={item.id} className="flex items-center justify-between gap-4 rounded-2xl border border-[var(--line)] bg-white p-5"><div>{kind === "trips" ? <Link href={`/trips/${item.id}`} className="font-semibold text-[var(--teal)] hover:underline">{item.name}</Link> : <h2 className="font-semibold">{`${item.resource_type} 價格通知`}</h2>}<p className="mt-1 text-sm text-[var(--muted)]">{item.total_price ? twd.format(item.total_price) : item.target_price ? `低於 ${twd.format(item.target_price)} 時提醒` : "持續追蹤價格"}</p></div><button onClick={() => remove(item.id)} aria-label="刪除" className="rounded-xl border border-[var(--line)] p-2 text-[var(--muted)]"><Trash2 size={17} /></button></article>)}</div>;
}

...
/** Sub-topics a tile shows before it offers the rest. */
export const TOPIC_TILE_SUBTOPICS = 3;

/**
 * A section's top-level topics as tiles: the label linking to the topic hub, the hub's
 * lead, and the first few sub-topics as chips. A topic nothing is published under is left
 * out unless the reader is on it, for the reason `TopicChips` gives: a tile that leads to
 * an empty page is worse than none. A topic whose count an older API did not send is kept.
 *
 * `topic.count` decides that visibility and nothing else -- the tile no longer prints it.
 * The number changes every time an article ships, so on the page it was permanently out of
 * date, and it answered a question nobody browsing a topic was asking.
 */
              ...
              {children.length ? (
                <ul className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  {children.length > TOPIC_TILE_SUBTOPICS ? (
                    <li className="text-[length:var(--text-meta)] text-[var(--muted)]">{labels.more}</li>
                  ) : null}
                </ul>
              ) : null}
            </li>

...
export function DiscoveryPreferenceEditor({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  if (result.data && !result.error) return <PreferenceForm key={`${result.data.version}:${revision}`} initial={result.data} onClose={onClose} onSaved={onSaved} onReload={() => { result.reload(); setRevision((value) => value + 1); }} />;
  return <Dialog title={c.preferences} onClose={onClose}>{result.error ? <p role="alert">{c.unavailable}</p> : <p role="status">{c.loading}</p>}</Dialog>;
}
function PreferenceForm({ initial, onSaved, onReload, onClose }: { initial: DiscoveryPreferences; onSaved: () => void; onReload: () => void; onClose: () => void }) {
  const locale = useLocale();
  const c = getDiscoveryCopy(locale);
  const feedback = getDiscoveryFeedback(locale);
  const closeCopy = calmCopy(locale);
  ...
  const dirty = destinations !== initial.destinations.join(", ") || topics !== initial.topics.join(", ") || saved !== initial.include_saved || following !== initial.include_following;
  function requestClose(proceed: () => void) {
    if (busy || (dirty && !window.confirm(`${closeCopy.dirtyHint}\n${closeCopy.discard}?`))) return false;
    proceed(); return true;
  }
  useNavigationGuard(dirty || busy, requestClose);
  ...
  return <Dialog title={c.preferences} onClose={() => { requestClose(onClose); }}><form onSubmit={(event) => void submit(event)} className="space-y-5"><p className="text-sm leading-7 text-[var(--muted)]">{c.preferenceHelp}</p>

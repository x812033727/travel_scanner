...
export function AdminDeploymentsPanel() {
  ...
  // Escape used to hang off the <form>'s onKeyDown, so it answered only while focus was
  // inside the form — and a click on the scrim leaves focus on <body>. The shared sheet
  // listens on the document, traps Tab, locks the page behind it and hands focus back to
  // the deploy button, including on the path where a started deploy closes this itself.
  const confirmRef = useModalSheet<HTMLFormElement>(confirmOpen, closeConfirmation);
  // After the sheet's own opening focus, so the password field is what a reader lands on.
  useEffect(() => {

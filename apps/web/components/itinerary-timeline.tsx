...
import { formatTime, groupTripItems, type TripItem } from "@/lib/trip-types";
...
export function ItineraryTimeline({ items }: { items: TripItem[] }) {
  return <div className="space-y-6">{groupTripItems(items).map(([day, rows], dayIndex) => <section key={day} className="rounded-[1.75rem] border border-[var(--line)] bg-white p-5 md:p-6">
    ...
    <ol className="relative space-y-3 before:absolute before:bottom-4 before:left-[1.15rem] before:top-4 before:w-px before:bg-[var(--line)]">{rows.map((item) => <li key={item.id} className="relative grid grid-cols-[2.3rem_1fr] gap-3"><span className={`z-10 mt-3 grid h-9 w-9 place-items-center rounded-full ${item.is_estimated ? "bg-amber-100 text-amber-800" : "bg-[var(--teal-soft)] text-[var(--teal)]"}`}>{item.is_estimated ? <Sparkles size={16} /> : <Clock3 size={16} />}</span><div className="rounded-2xl border border-[var(--line)] bg-[var(--paper)] p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-xs font-semibold text-[var(--muted)]">{formatTime(item.start_time)}{item.end_time ? `–${formatTime(item.end_time)}` : ""}</p><h3 className="mt-1 font-semibold">{item.title}</h3></div><div className="flex gap-2">{item.locked && <span title="固定項目" className="rounded-full bg-white p-1.5 text-[var(--teal)]"><LockKeyhole size={14} /></span>}{item.is_estimated && <span className="rounded-full bg-amber-100 px-2 py-1 text-[.68rem] font-semibold text-amber-900">估算</span>}</div></div>{item.location_name && <p className="mt-2 flex items-center gap-1.5 text-sm text-[var(--muted)]"><MapPin size={14} />{item.location_name}</p>}</div></li>)}</ol>
  </section>)}</div>;
}

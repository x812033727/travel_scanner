...
export function ItineraryTimeline({
  ...
  return (
    <div className="space-y-6">
      {groupTripItems(items).map(([day, rows], dayIndex) => (
        <section
          key={day}
              ...
              return (
                <li key={item.id}>
                  <div className="relative grid grid-cols-[2.3rem_1fr] gap-3">
                    <span
                      className={`z-10 mt-3 grid h-9 w-9 place-items-center rounded-full ${item.is_estimated ? "bg-amber-100 text-amber-800" : "bg-[var(--teal-soft)] text-[var(--teal)]"}`}
                    >
                      {item.is_estimated ? (
                        <Sparkles size={16} />
                      ) : (
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-2">
                        ...
                        <div className="flex gap-2">
                          {item.locked && (
                            <span
                              title="固定項目"
                              className="rounded-full bg-white p-1.5 text-[var(--teal)]"
                            >
                              <LockKeyhole size={14} />
                            </span>
                          )}
                          {item.is_estimated && (
                            <span className="rounded-full bg-amber-100 px-2 py-1 text-[.68rem] font-semibold text-amber-900">
                              估算
                            </span>
                          )}
                        </div>
                  ...
                  {route && (
                    <div className="mt-3">
                      <RouteSegmentCard segment={route} />
                    </div>
                  )}
                </li>
              );

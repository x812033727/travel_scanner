...
export function FlightStatusSearch() {
  ...
  return <main className="mx-auto max-w-5xl px-5 pb-20 md:px-8">
    ...
    <form onSubmit={submit} className="relative -mt-5 mx-3 rounded-3xl border border-[var(--line)] bg-white p-5 shadow-[var(--shadow-lg)] md:mx-8 md:p-7">
      <div className="mb-5 flex gap-2" role="tablist"><button type="button" onClick={() => setMode("ident")} className={`min-h-11 rounded-full px-4 py-2 text-sm font-semibold ${mode === "ident" ? "bg-[var(--teal)] text-white" : "bg-[var(--paper)]"}`}>依班號</button><button type="button" onClick={() => setMode("route")} className={`min-h-11 rounded-full px-4 py-2 text-sm font-semibold ${mode === "route" ? "bg-[var(--teal)] text-white" : "bg-[var(--paper)]"}`}>依航線</button></div>

...
export function FlightStatusSearch() {
  ...
  return <main className="mx-auto max-w-5xl px-5 pb-20 md:px-8">
    ...
    <form onSubmit={submit} className="relative -mt-5 mx-3 rounded-3xl border border-[var(--line)] bg-white p-5 shadow-[var(--shadow-lg)] md:mx-8 md:p-7">
      {/* Not tabs: there are no tabpanels, and role="tablist" over two plain buttons
          told a screen reader to expect an arrow-key widget that does not exist.
          Two toggles that announce which one is on is what this actually is. */}
      <div className="mb-5 flex gap-2"><button type="button" aria-pressed={mode === "ident"} onClick={() => setMode("ident")} className={`min-h-11 rounded-full px-4 py-2 text-sm font-semibold ${mode === "ident" ? "bg-[var(--teal)] text-white" : "bg-[var(--paper)]"}`}>依班號</button><button type="button" aria-pressed={mode === "route"} onClick={() => setMode("route")} className={`min-h-11 rounded-full px-4 py-2 text-sm font-semibold ${mode === "route" ? "bg-[var(--teal)] text-white" : "bg-[var(--paper)]"}`}>依航線</button></div>
      <div className="grid gap-4 md:grid-cols-[1fr_1fr_auto]">
        ...
        {session.status === "signed_out"
          ? <Link href={loginPath(pathname)} className="mt-auto flex items-center justify-center gap-2 rounded-xl bg-[var(--coral-fill)] px-6 py-3 font-semibold text-white"><LogIn size={18} />{usage("signInToUse")} · {charge.label}</Link>
          : <button disabled={busy || charge.status !== "ready" || session.status === "loading"} className="mt-auto flex items-center justify-center gap-2 rounded-xl bg-[var(--coral-fill)] px-6 py-3 font-semibold text-white disabled:opacity-60">{busy ? <LoaderCircle size={18} className="animate-spin" /> : <Search size={18} />}查詢 · {charge.label}</button>}
      </div>

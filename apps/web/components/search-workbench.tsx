...
/** A chosen chip carries a tick as well as the tint: colour alone is not a state. */
function Chosen({ active }: { active: boolean }) {
  return active ? <Check size={15} aria-hidden className="shrink-0 text-[var(--teal-dark)]" /> : null;
}
...
export function SearchWorkbench({ compact = false }: { compact?: boolean }) {
  ...
  // The API answers in the reader's locale already; the offline list is localized here.
  const dynamicCities = useMemo<DestinationCity[]>(() => catalog.filter((item) => item.searchable).map((item) => ({
    id: item.id, country: item.country_code, name: item.city, airport: item.primary_gateway, summary: item.reason,
    recommendedDays: { min: item.recommended_days.min, max: item.recommended_days.max }, areas: item.areas,
    tags: item.role === "secondary" ? [SECONDARY_CITY_TAG] : [], timezone: item.timezone, currency: item.currency,
  })), [catalog]);
  const offlineCities = useMemo(() => localizeDestinations(tc), [tc]);
  const availableCities = useMemo(
    () => (dynamicCities.length ? dynamicCities : offlineCities).filter((item) => countriesSelected.includes(item.country)),
    [countriesSelected, dynamicCities, offlineCities],
  );
  ...
  return <form id="trip-search" onSubmit={submit} data-compact={compact || undefined} data-advanced={advancedOpen || undefined} className="frontend-search-workbench rounded-[2rem] border border-[var(--line)] bg-[var(--surface)] p-5 shadow-[var(--shadow-lg)] md:p-7">
    {compact && <div className="frontend-search-intro"><h2 className="text-xl font-bold">{copy.basics}</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{copy.searchHelp}</p></div>}
    ...
    <ol className="mb-6 gap-1 max-sm:-mx-1 max-sm:flex max-sm:snap-x max-sm:overflow-x-auto max-sm:px-1 max-sm:[contain:inline-size] max-sm:[scrollbar-width:none] sm:grid sm:grid-cols-5" aria-label={t("stepsLabel")}>{stepKeys.map((key, index) => { const label = t(`steps.${key}`); return <li key={key} className="max-sm:flex-none max-sm:snap-start"><button type="button" aria-current={index === step ? "step" : undefined} aria-label={index < step ? t("stepGoTo", { label }) : label} disabled={index > step} onClick={() => { if (index <= step) setStep(index); }} className={`min-h-11 w-full rounded-xl px-1 py-2 text-center text-xs font-semibold transition max-sm:whitespace-nowrap max-sm:px-3 ${index === step ? "bg-[var(--teal)] text-white" : index < step ? "bg-[var(--teal-soft)] text-[var(--teal-dark)]" : "cursor-default bg-[var(--paper)] text-[var(--muted)]"}`}><span className="hidden sm:inline">{index + 1}. </span>{label}</button></li>; })}</ol>
    ...
    <section className={step === 1 ? "block" : "hidden"}><h3 ref={step === 1 ? stepHeadingRef : undefined} tabIndex={-1} className="flex items-center gap-2 text-lg font-bold outline-none"><MapPin size={19} />{t("destinationTitle")}</h3><p className="mt-1 text-sm text-[var(--muted)]">{t("destinationHint")}</p><div className="mt-4 grid grid-cols-3 gap-2">{countryKeys.map((key) => <button key={key} type="button" aria-pressed={countriesSelected.includes(key)} onClick={() => toggleCountry(key)} className={optionClass(countriesSelected.includes(key))}><span className="flex items-center gap-1.5"><Chosen active={countriesSelected.includes(key)} /><strong>{tc(`countries.${key}.name`)}</strong></span><span className="mt-1 hidden text-xs sm:block">{tc(`countries.${key}.caption`)}</span></button>)}</div><label className="mt-4 block text-sm font-semibold">{t("cityLabel")}<select aria-label={t("citySelect")} value={destinationCode} onChange={(event) => { setDestinationCode(event.target.value); setSelectedExtensions([]); }} className={fieldClass}><option value="">{t("cityAny")}</option><optgroup label={t("cityGroupPrimary")}>{availableCities.filter((city) => !city.tags.includes(SECONDARY_CITY_TAG)).map((city) => <option key={city.id} value={city.airport}>{city.name} · {city.airport}</option>)}</optgroup><optgroup label={t("cityGroupSecondary")}>{availableCities.filter((city) => city.tags.includes(SECONDARY_CITY_TAG)).map((city) => <option key={city.id} value={city.airport}>{city.name} · {city.airport}</option>)}</optgroup></select>{countriesSelected.length === 0 && <span className="mt-1 block text-xs font-normal text-[var(--muted)]">{t("cityNeedsCountry")}</span>}</label>{availableExtensions.length > 0 && <fieldset className="mt-4 rounded-2xl border border-violet-200 bg-violet-50 p-4"><legend className="px-1 text-sm font-semibold">{t("extensionsLegend")}</legend><div className="mt-2 flex flex-wrap gap-2">{availableExtensions.map((item) => <label key={item.id} className="flex min-h-11 items-center gap-2 rounded-xl bg-white px-3 py-2 text-sm"><input type="checkbox" className="h-5 w-5 accent-[var(--teal)]" checked={selectedExtensions.includes(item.id)} onChange={(event) => setSelectedExtensions((current) => event.target.checked ? [...current, item.id].slice(0, 2) : current.filter((id) => id !== item.id))} />{item.city}</label>)}</div></fieldset>}</section>

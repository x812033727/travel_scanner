...
export function SearchWorkbench() {
  ...
  return <form id="trip-search" onSubmit={submit} className="rounded-[2rem] border border-[var(--line)] bg-white p-5 shadow-[var(--shadow-lg)] md:p-7">
    ...
    <ol className="mb-6 gap-1 max-sm:-mx-1 max-sm:flex max-sm:snap-x max-sm:overflow-x-auto max-sm:px-1 max-sm:[scrollbar-width:none] sm:grid sm:grid-cols-5" aria-label={t("stepsLabel")}>{stepKeys.map((key, index) => { const label = t(`steps.${key}`); return <li key={key} className="max-sm:flex-none max-sm:snap-start"><button type="button" aria-current={index === step ? "step" : undefined} aria-label={index < step ? t("stepGoTo", { label }) : label} disabled={index > step} onClick={() => { if (index <= step) setStep(index); }} className={`min-h-11 w-full rounded-xl px-1 py-2 text-center text-xs font-semibold transition max-sm:whitespace-nowrap max-sm:px-3 ${index === step ? "bg-[var(--teal)] text-white" : index < step ? "bg-[var(--teal-soft)] text-[var(--teal-dark)]" : "cursor-default bg-[var(--paper)] text-[var(--muted)]"}`}><span className="hidden sm:inline">{index + 1}. </span>{label}</button></li>; })}</ol>

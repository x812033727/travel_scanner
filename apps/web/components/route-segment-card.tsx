...
export function RouteSegmentCard({
  ...
  return <article className={`route-detail-card ${selected ? "route-detail-card-selected" : ""}`}>
    ...
    <div className="route-time-grid mt-4" aria-label="移動時間摘要"><div><span>出發</span><strong>{formatTime(segment.departure_time)}</strong></div><Route size={18} aria-hidden="true" /><div><span>抵達</span><strong>{formatTime(segment.arrival_time)}</strong></div><div className="route-buffer-summary"><span>預留</span><strong>＋{segment.buffer_minutes || 0} 分</strong></div><div className="route-ready-summary"><span>下一站可開始</span><strong>{formatTime(segment.ready_time)}</strong></div></div>
    ...
    {expanded && <div className="mt-4 space-y-4 border-t border-[var(--line)] pt-4">
      ...
      <footer className="flex flex-wrap items-center justify-between gap-2 border-t border-[var(--line)] pt-3 text-xs text-[var(--muted)]"><span>{segment.attribution} · {new Date(segment.generated_at).toLocaleString("zh-TW")}</span>{segment.maps_url && <a href={segment.maps_url} target="_blank" rel="noreferrer" className="flex min-h-11 items-center gap-1 font-semibold text-[var(--teal)]">用 Google Maps 開啟<ExternalLink size={13} /></a>}</footer>
    </div>}
  </article>;
}

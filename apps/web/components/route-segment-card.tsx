...
export function RouteSegmentCard({
  ...
  const manual = segment.provider === "manual" || segment.status === "manual";
  const estimated = !manual && (segment.provider === "estimate" || segment.status === "estimated" || String(segment.schedule_mode) === "estimate");
  const sourced = !manual && !estimated;
  const steps = sourced ? segment.steps : [];
  ...
  const mapsLabel = segment.provider === "naver_maps" || segment.provider === "odsay" || /NAVER|ODsay/i.test(segment.attribution)
    ? t("openInNaver")
    : t("openInGoogle");
  const navigations = segment.external_navigations?.filter((navigation) => navigation.travel_mode === mode);
  const timingLabel = manual ? t("transportUx.manualReserved") : estimated ? t("estimatedTiming") : segment.attribution;
  ...
  return <article className={`route-detail-card ${selected ? "route-detail-card-selected" : ""}`}>
    <button type="button" aria-expanded={expanded} onClick={() => { setExpanded(!expanded); onSelect?.(); }} className="flex min-h-14 w-full items-start justify-between gap-3 text-left">
      <span className="flex min-w-0 items-start gap-3"><span className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl ${modeMeta.tone}`}><ModeIcon size={20} /></span><span className="min-w-0"><span className="flex flex-wrap items-center gap-2"><strong className="text-base">{t(modeMeta.key)} · {t(manual ? "transportUx.manualDuration" : "durationMinutes", { minutes: segment.duration_minutes })}</strong>{segment.is_override && <span className="route-override-badge">{t("overrideBadge")}</span>}{expired && <span className="route-stale-badge">{t("staleBadge")}</span>}</span><span className="mt-1 block truncate text-xs text-[var(--muted)]">{manual || estimated ? timingLabel : lines || distanceLabel(t, segment.distance_meters) || t("noSteps")}</span></span></span>
      <ChevronDown size={17} className={`mt-3 shrink-0 transition ${expanded ? "rotate-180" : ""}`} />
    </button>

    <p className="mt-3 text-sm font-semibold text-[var(--teal)]">{t("mapTimeSource", { provider: timingLabel })}</p>
    {sourced && segment.schedule_mode === "preview" && <p className="route-reference-time mt-2 rounded-xl bg-sky-50 p-3 text-sm font-semibold text-sky-900">{t("transportUx.referenceTimes")}</p>}
    {(manual || estimated) && <p className="mt-1 text-xs leading-5 text-[var(--muted)]">{t("transportUx.manualScheduleHint")}</p>}
    ...
    {expanded && <div className="mt-4 space-y-4 border-t border-[var(--line)] pt-4">
      ...
      {!steps.length && <p className="rounded-xl bg-[var(--paper)] px-3 py-3 text-sm text-[var(--muted)]">{t(sourced ? "noSteps" : "transportUx.stepsMissing")}</p>}
      <div className="route-detail-metrics grid grid-cols-2 gap-2 sm:grid-cols-4"><div className="route-metric"><Clock3 size={15} /><span>{t("metricTravel")}</span><strong>{t("metricMinutes", { minutes: segment.duration_minutes })}</strong></div><div className="route-metric"><MapPin size={15} /><span>{t("metricDistance")}</span><strong>{distanceLabel(t, segment.distance_meters) || t("notProvided")}</strong></div><div className="route-metric"><BusFront size={15} /><span>{t("metricTransfers")}</span><strong>{transitSteps.length ? t("transfersCount", { count: transitSteps.length - 1 }) : t("transportUx.transfersUnknown")}</strong></div><div className="route-metric"><CircleDollarSign size={15} /><span>{t("metricFare")}</span><strong>{segment.fare != null ? `${segment.currency || ""} ${String(segment.fare)}` : t("notProvided")}</strong></div></div>
      {sourced && transitSteps.length > 0 && !segment.details_available.includes("exit") && mode === "transit" && <p className="flex items-center gap-2 rounded-xl bg-[var(--paper)] px-3 py-2 text-xs text-[var(--muted)]"><MapPin size={14} />{t("noExit")}</p>}
      <footer className="flex flex-wrap items-center justify-between gap-2 border-t border-[var(--line)] pt-3 text-xs text-[var(--muted)]">
        <span>{segment.attribution} · {new Date(segment.generated_at).toLocaleString(locale)}</span>
        {showNavigation && (navigations ? navigations.map((navigation) => <div key={navigation.provider} className="flex flex-wrap items-center gap-3">
          <a href={safeExternalHref(navigation.web_url)} target="_blank" rel="noopener noreferrer" className="flex min-h-11 items-center gap-1 font-semibold text-[var(--teal)]">{t("navigateWithProvider", { provider: navigation.label })}<ExternalLink size={13} /></a>
          {navigation.provider === "naver_maps" && navigation.app_url !== navigation.web_url && <a href={safeExternalHref(navigation.app_url, ["nmap:", "https:"])} className="flex min-h-11 items-center gap-1 font-semibold text-[var(--teal)]">{t("openNaverApp")}<ExternalLink size={13} /></a>}
        </div>) : segment.maps_url && <a href={safeExternalHref(segment.maps_url)} target="_blank" rel="noopener noreferrer" className="flex min-h-11 items-center gap-1 font-semibold text-[var(--teal)]">{mapsLabel}<ExternalLink size={13} /></a>)}
      </footer>
    </div>}
  </article>;
}

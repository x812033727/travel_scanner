...
const planningModes = ["ai_draft", "manual_blank"] as const;
const routePreferences = ["FEWER_TRANSFERS", "FASTEST", "LESS_WALKING"] as const;
type LodgingMode = (typeof lodgingModes)[number];
type Pace = (typeof paces)[number];
type PlanningMode = (typeof planningModes)[number];

const stepKeys = ["basics", "travelers", "stay", "review"] as const;
// Same cap as SaveTripRequest on the API: (end - start).days <= 60.
...
// The API stores every amount as an integer, so a typed decimal is rounded
// here instead of failing validation on a field that may be on a hidden step.
...
export function NewTripForm() {
  ...
  // A fresh object per failure re-runs the focus effect even when the same
  // message repeats, so pressing Next twice announces the problem again.
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      if (draft) {
        ...
        // A draft saved before midnight may now start in the past; drop the
        // dates and return to the first step so the calendar asks again.
        const datesValid = Boolean(start && end) && start >= today && end >= start && dayCount(start, end) <= MAX_TRIP_DAYS;
        setForm((current) => {
          const next = { ...current };
        ...
        if (typeof draft.step === "number") setStep(datesValid ? Math.min(Math.max(draft.step, 0), stepKeys.length - 1) : 0);
      }
      setDraftReady(true);
    }, 0);
  ...
  const reviewDetails = [
    [t("review.labels.travelers"), t("review.values.travelers", { count: travelerCount, rooms: Number(form.rooms) })],
    ...
    [t("review.labels.reviewScore"), form.min_review_score ? t("stay.scorePlus", { score: Number(form.min_review_score).toFixed(1) }) : noPreference],
  ...
  // noValidate: steps are hidden with display:none, and a browser cannot focus
  // an invalid control inside one, so native validation would block the submit
  // silently; validate() already covers every rule.
  return <form noValidate onSubmit={submit} className="grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
    <section className="rounded-[2rem] border border-[var(--line)] bg-white p-6 shadow-[var(--shadow-lg)] md:p-8">
      ...
      <ol aria-label={t("stepsLabel")} className="mb-7 grid grid-cols-4 gap-1">{stepKeys.map((key, index) => <li key={key} aria-current={index === step ? "step" : undefined} className={`rounded-xl px-1 py-2 text-center text-xs font-semibold ${index === step ? "bg-[var(--teal)] text-white" : index < step ? "bg-[var(--teal-soft)] text-[var(--teal-dark)]" : "bg-[var(--paper)] text-[var(--muted)]"}`}><span className="hidden sm:inline">{index + 1}. </span>{t(`steps.${key}`)}</li>)}</ol>
    ...
    <aside className="rounded-[2rem] bg-[var(--ink)] p-6 text-white md:p-8">
      <p className="text-xs font-semibold tracking-[.18em] text-emerald-200">{t("summary.eyebrow")}</p><h2 className="mt-2 text-2xl font-bold">{form.destination_name.trim() || t("summary.noDestination")}</h2><div className="mt-5 flex flex-wrap gap-2">{summary.map((item, index) => <span key={`${index}-${item}`} className="rounded-full bg-white/10 px-3 py-1.5 text-xs text-white/80">{item}</span>)}</div>

...
export function PetRequirementFields({ value, onChange }: { value: PetRequirements; onChange: (value: PetRequirements) => void }) {
  ...
  return <fieldset className="space-y-4"><legend className="mb-3 font-bold">{t("petCompanion")}</legend><div className="grid gap-3 sm:grid-cols-4">
    <label className="text-sm font-semibold">{t("species")}<input required list={speciesId} className={fieldClass} maxLength={40} value={value.species} onChange={(e) => onChange({ ...value, species: e.target.value })} /><datalist id={speciesId}><option value="dog">{t("dog")}</option><option value="cat">{t("cat")}</option></datalist></label>

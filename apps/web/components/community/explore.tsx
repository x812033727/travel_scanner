...
export function TravelExplore() {
 ...
 return <div className="grid gap-4 sm:grid-cols-2"><Link className={panelClass} href="/#trip-search">{tc("startPlanning")}</Link>{primaryNavLinks.filter((item) => !item.feature || featureVisible(visibility, item.feature)).map((item) => <Link className={panelClass} key={item.href} href={item.href}>{t(item.key)}</Link>)}{flags.enabled && <><Link className={panelClass} href="/community/search">{tc("searchCommunity")}</Link><Link className={panelClass} href="/pet-friendly">{tc("pets")}</Link></>}</div>;
}

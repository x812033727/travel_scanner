...
export function PetAdmin() {
 ...
 return <AdminGate><div className="space-y-6"><h1 className="text-3xl font-bold">{t("adminPets")}</h1><AdminReadOnlyNotice capability="community.manage" /><Tabs label={t("adminPets")} value={tab} onChange={setTab} items={["places","reports","coverage"].map((value)=>({value,label:t(`petAdminTabs.${value}`)}))}>{tab==="places" && <PlacesReview canManage={manage.allowed} />}{tab==="reports" && <PetReportReview canManage={manage.allowed} />}{tab==="coverage" && <Coverage />}</Tabs></div></AdminGate>;
}
function PlacesReview({canManage}:{canManage:boolean}) {
 const t=useTranslations("community");
...
function PetReportReview({canManage}:{canManage:boolean}) {
 const t=useTranslations("community");
 ...
 return <div className="space-y-4"><ErrorNotice error={data.error} />{data.data?.items.length===0 && <Empty>{t("queueEmpty")}</Empty>}{data.data?.items.map((row)=><article key={row.id} className={`${panelClass} space-y-3`}><Link href={`/pet-friendly/${row.place_id}`} className="underline">{t("viewRules")}</Link><p className="whitespace-pre-wrap break-words">{row.body}</p>{row.visited_on && <p>{t("visitedOn")}: {row.visited_on}</p>}{row.source_url && <a href={row.source_url} target="_blank" rel="noopener noreferrer" className="underline">{t("ruleSource")}</a>}<div className="grid grid-cols-2 gap-3">{row.media_ids.map((id)=><CommunityImage key={id} id={id} alt={t("visitPhoto")} review />)}</div><Rules rules={row.proposed_policies} /><p className="text-sm">{t("experienceNotice")}</p><div className="flex gap-2">{["resolved","dismissed"].map((status)=><Button key={status} secondary disabled={!canManage} onClick={()=>setAction({id:row.id,status})}>{t(`states.${status}`)}</Button>)}<Button secondary disabled={!canManage} onClick={()=>setAction({id:row.id,status:"resolved",flag_conflict:true})}>{t("flagConflict")}</Button></div></article>)}{action && <ReasonAction title={t(`states.${action.status}`)} onClose={()=>setAction(undefined)} onSubmit={async(reason)=>{if(!canManage)return;await api(`/admin/pet-friendly/reports/${action.id}`,{method:"PUT",body:JSON.stringify({...action, id:undefined,reason})});await data.reload();}} />}</div>;
}
function Coverage() {
 const t=useTranslations("community");

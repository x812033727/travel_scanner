...
export function SharedTripView({ token }: { token: string }) {
  ...
  return <main className="mx-auto max-w-4xl px-5 pb-20 md:px-8"><section className="mb-6 rounded-[2rem] border border-[var(--line)] bg-white p-6 shadow-[var(--shadow-lg)] md:p-8"><p className="flex items-center gap-2 text-sm font-semibold text-[var(--teal)]"><ShieldCheck size={17} />唯讀分享旅程</p><h1 className="mt-2 text-3xl font-bold md:text-4xl">{trip.name}</h1><p className="mt-3 text-[var(--muted)]">預估總額 {twd.format(Number(trip.total_price))} · 最後更新 {trip.updated_at ? new Date(trip.updated_at).toLocaleString("zh-TW") : "近期"}</p><p className="mt-4 rounded-xl bg-[var(--teal-soft)] p-3 text-sm text-[var(--teal-dark)]">價格與庫存可能變動；離開本站預訂前請向供應商再次確認。</p></section><ItineraryTimeline items={trip.items} /></main>;
}

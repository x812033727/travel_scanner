    ...
    {usage.available && <>
      ...
      <div className="mt-5">
        ...
        <div className="mt-3 overflow-x-auto rounded-xl border border-[var(--line)] bg-white">
          <table className="admin-responsive-table min-w-[42rem] w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-[var(--line)]">{usage.monthly_history.map((month) => <tr key={month.period}><th className="px-3 py-2.5 font-semibold">{month.period}</th><td data-label={t("settingsPanel.thRequests")} className="px-3 py-2.5 tabular-nums">{numberFormat.format(month.used)}</td><td data-label={t("settingsPanel.freeUsage")} className="px-3 py-2.5 tabular-nums">{numberFormat.format(month.free_usage)}</td><td data-label={t("settingsPanel.skuRemaining")} className="px-3 py-2.5 tabular-nums">{numberFormat.format(month.free_remaining)}</td><td data-label={t("settingsPanel.thOverage")} className={`px-3 py-2.5 tabular-nums ${month.billable_overage ? "font-semibold text-red-700" : ""}`}>{numberFormat.format(month.billable_overage)}</td></tr>)}</tbody>
          </table>
        </div>
      </div>

  ...
  return (
    <article className="overflow-hidden rounded-[1.5rem] border border-[var(--line)] bg-white">
      ...
      <div className="p-5">
        ...
        {offer.attributions?.length ? <p className="mt-1 text-xs text-[var(--muted)]">圖片：{offer.attributions.map((label, index) => offer.attribution_urls?.[index] ? <span key={`${label}-${index}`}>{index > 0 ? "、" : ""}<a className="underline" href={safeExternalHref(offer.attribution_urls[index])} target="_blank" rel="noreferrer">{label}</a></span> : <span key={`${label}-${index}`}>{index > 0 ? "、" : ""}{label}</span>)}</p> : null}
        <a href={safeExternalHref(actionUrl)} target="_blank" rel="noopener noreferrer" className="mt-5 flex items-center justify-center gap-2 rounded-xl border border-[var(--teal)] px-4 py-3 text-sm font-semibold text-[var(--teal)]">{offer.action_kind === "deep_link" ? "前往供應商" : "外站重新確認"}<ExternalLink size={16} /></a>

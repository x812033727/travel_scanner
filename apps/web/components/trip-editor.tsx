...
export function TripEditor({ tripId }: { tripId: string }) {
  ...
  async function applyAIItinerary() {
    ...
    try {
      ...
      // "Built-in fallback applied, not charged" is true of a spent planner budget too, but it
      // reads as an outage and invites another try that cannot reach AI either.
      setNotice(updated.usage?.status === "charged"
        ? te("aiAppliedCharged", { provider, scope: scopeLabel, uses: updated.usage.uses })
        : updated.planning?.warnings.includes("planner_budget_reached")
          ? te("aiBudgetReached")
  ...
  // A spent planner budget comes back as a catalogue preview that is otherwise identical to
  // an outage. Taken for a glitch, it sends the traveller straight back to Generate, and the
  // preview limiter does not return a slot for a fallback, so the reason is shown here,
  // before Apply, rather than only in the closed review panel afterwards.
  const previewBudgetSpent = Boolean(aiPreview?.planning.warnings.includes("planner_budget_reached"));
  // The headline already says the budget ran out, and that code's own reminder line talks about
  // adjusting a plan nobody can touch before Apply, so the list under it keeps only the other
  // reasons, such as blank slots.
  const previewBudgetReminders = previewBudgetSpent && aiPreview
    ? plannerWarningTexts(aiPreview.planning.warnings.filter((code) => code !== "planner_budget_reached"), te)
    : [];

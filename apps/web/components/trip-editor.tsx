...
export function TripEditor({ tripId }: { tripId: string }) {
  ...
  async function regenerateItinerary() {
    if (!trip) return;
    if (!window.confirm("AI 會替換未鎖定的舊 AI 建議；手動加入、已鎖定及固定預約項目會保留。要繼續嗎？")) return;
    const currentTrip = await save(false);
    if (!currentTrip) return;
    setBusy(true); setError(undefined); setNotice(undefined);
    try {
      const updated = await api<Trip>(`/trips/${currentTrip.id}/itinerary/generate`, { method: "POST", headers: { "Idempotency-Key": crypto.randomUUID() }, body: JSON.stringify({ version: currentTrip.version }) });
      setTrip(updated); setItems(updated.items); setRoutes([]); setSelectedRoute(undefined);
      setNotice(updated.usage?.status === "charged" ? "AI 已重新排行程並扣除 1 次；手動、鎖定與固定預約項目已保留。" : "已使用內建備援重新排行程，本次未扣次；手動、鎖定與固定預約項目已保留。");
    } catch (reason) {
      if (isUsageInsufficient(reason)) { router.push("/pricing"); return; }
      setError(`${(reason as Error).message}；原行程內容未變更。`);
    } finally { setBusy(false); }
  }
  ...
  return <main className="mx-auto max-w-7xl px-5 pb-20 md:px-8">
    <section className="mb-6 rounded-[2rem] border border-[var(--line)] bg-white p-6 shadow-[var(--shadow-lg)] md:p-8">
      <div className="flex flex-wrap items-start justify-between gap-5"><div><p className="text-sm font-semibold text-[var(--teal)]">行程規劃器 · 版本 {trip.version}</p><h1 className="mt-2 text-3xl font-bold md:text-4xl">{trip.name}</h1><p className="mt-3 text-[var(--muted)]">{trip.destination_name || "旅程"}{trip.start_date ? ` · ${trip.start_date} 至 ${trip.end_date}` : ""}{Number(trip.total_price) > 0 ? ` · ${twd.format(Number(trip.total_price))}` : ""}</p><p className="mt-2 text-xs text-[var(--muted)]">首次 AI 草稿免費；真實 AI 重排成功扣 1 次，內建備援不扣次。</p><div className="max-w-xs"><PriceAlertButton resourceType="trip" resourceId={trip.id} currentPrice={Number(trip.total_price)} currency={trip.currency} returnPath={`/trips/${trip.id}`} /></div></div><div className="flex flex-wrap gap-2"><button onClick={reoptimizePrices} disabled={busy || trip.mode === "manual"} className="flex items-center gap-2 rounded-xl border border-[var(--line)] px-4 py-3 text-sm font-semibold disabled:opacity-40"><RefreshCw size={16} />重新查價</button><button onClick={regenerateItinerary} disabled={busy} className="flex items-center gap-2 rounded-xl border border-violet-300 px-4 py-3 text-sm font-semibold text-violet-800"><Sparkles size={16} />AI 重新排行程</button><button onClick={() => optimize()} disabled={busy} className="flex items-center gap-2 rounded-xl border border-[var(--teal)] px-4 py-3 text-sm font-semibold text-[var(--teal)]"><RouteIcon size={16} />最佳化整趟動線</button><button onClick={() => save()} disabled={busy} className="flex items-center gap-2 rounded-xl bg-[var(--teal)] px-4 py-3 text-sm font-semibold text-white"><Save size={16} />{busy ? "處理中…" : "儲存變更"}</button></div></div>
      {trip.planning && <div className={`mt-5 rounded-2xl p-4 text-sm ${trip.planning.status === "fallback" ? "bg-amber-50 text-amber-900" : "bg-emerald-50 text-emerald-900"}`}><p className="font-semibold">{trip.planning.status === "fallback" ? "已使用內建備援草稿" : "AI 已排好可編輯初稿"}</p><p className="mt-1 text-xs leading-5 opacity-80">來源：{trip.planning.provider === "catalog" ? "內建目的地資料" : `${trip.planning.provider}${trip.planning.model ? ` · ${trip.planning.model}` : ""}`}。景點、營業時間與預約需求仍需確認。</p>{trip.planning.warnings.length > 0 && <ul className="mt-2 list-disc pl-5 text-xs">{trip.planning.warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul>}</div>}

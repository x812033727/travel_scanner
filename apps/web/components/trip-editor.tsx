  ...
  return <main data-planner-theme={plannerTheme} className="planner-app-shell mx-auto max-w-7xl px-4 pb-36 sm:px-5 md:px-8 lg:pb-20">
    ...
    <PlannerOverlay open={Boolean(editingItem)} onClose={closeEditor} title={editingMeal ? `編輯${editingItem?.system_role === "lunch" ? "午餐" : "晚餐"}` : draftItem ? "新增安排" : "編輯安排"} description={editingMeal ? "固定時間與停留長度由全程餐食設定管理；這裡只調整餐廳與備註。" : draftItem ? "先填寫名稱；確認加入前不會修改目前行程。" : "修改會自動儲存；變更地點或時間後需重新查路。"} footer={draftItem && <div className="flex gap-3"><button type="button" onClick={closeEditor} className="min-h-12 flex-1 rounded-xl border border-[var(--line)] font-semibold">取消</button><button type="button" onClick={commitDraftItem} disabled={!draftItem.title.trim()} className="min-h-12 flex-[1.4] rounded-xl bg-[var(--teal)] px-4 font-semibold text-white disabled:opacity-40">加入行程</button></div>}>{editingItem && <div className="grid gap-5">
      ...
      {!editingItem.system_role && <div className="grid gap-4">
        <label className="text-sm font-semibold">日期<select value={editingItem.day_date} onChange={(event) => { const previousDay = editingItem.day_date; patchItem(editingItem.id, { day_date: event.target.value, start_time: editingItem.start_time ? withTime(event.target.value, timeValue(editingItem.start_time)) : null, end_time: null }); setActiveDay(event.target.value); if (!draftItem) setStaleDays((current) => new Set([...current, previousDay, event.target.value])); }} className={fieldClass}>{days.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
    ...
    <PlannerOverlay open={Boolean(flightRole)} onClose={() => { if (!action) setFlightRole(undefined); }} title={`設定${flightRole === "return_flight" ? "回程" : "去程"}航班`} description="起降時間會依各機場的當地時間原樣保存；不會擅自換算時區，也不會加入市區路線。" footer={<div className="flex gap-3"><button type="button" onClick={() => setFlightRole(undefined)} disabled={Boolean(action)} className="min-h-12 flex-1 rounded-xl border border-[var(--line)] font-semibold disabled:opacity-40">取消</button><button type="button" onClick={() => void saveFlightAnchor()} disabled={Boolean(action) || !flightDraftReady} className="flex min-h-12 flex-[1.4] items-center justify-center gap-2 rounded-xl bg-sky-800 px-4 font-semibold text-white disabled:opacity-45">{action?.startsWith("flight-") ? <Loader2 size={17} className="animate-spin" /> : <Check size={17} />}儲存航班</button></div>}>
      <div className="grid gap-5">
        ...
        {editingFlightItem && flightAnchorInfo(editingFlightItem) && <button type="button" onClick={() => void saveFlightAnchor(true)} disabled={Boolean(action)} className="min-h-11 rounded-xl border border-slate-300 bg-slate-50 px-4 text-sm font-semibold text-slate-700 disabled:opacity-40">清除並恢復為尚未設定</button>}
      </div>
    </PlannerOverlay>
  </main>;
}

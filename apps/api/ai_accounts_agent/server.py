...
from ai_accounts_agent.antigravity import SUBSCRIPTION_METHOD as AGY_SUBSCRIPTION
from ai_accounts_agent.antigravity import AntigravityAccounts
...
PROBED_USAGE = {"claude": "claude.ai", "agy": AGY_SUBSCRIPTION}
...
class AgentApplication:
    def __init__(
        ...
        *,
        ...
        self.accounts: dict[str, Accounts] = {
            ...
            "agy": agy or AntigravityAccounts(config),
        }
    ...
    def ttl_for(self, tool: str) -> float:
        return {
            "claude": self.config.claude_cache_seconds,
            "codex": self.config.codex_cache_seconds,
            "agy": self.config.agy_cache_seconds,
        }[tool]
    ...
    def slot_view(self, tool: str, slot: str, status: dict[str, Any]) -> dict[str, Any]:
        ...
        view["usage_refreshing"] = self.usage_refreshing(tool, slot)
        return view

    # --- plan usage (Claude, Antigravity) ---------------------------------------------

    def usage_refreshing(self, tool: str, slot: str) -> bool:
        with self._usage_lock:
            return (tool, slot) in self._usage_running
    ...
    def maybe_refresh_usage(
        ...
        *,
        ...
        """Start a background usage probe for a signed-in subscription when due.
        ...
        if method is None:
            return False
        if status.get("logged_in") is not True or status.get("auth_method") != method:
            return False
        if not after_login and self.logins.active_for(tool, slot) is not None:
        ...
        with self._usage_lock:
            ...
            if not force:
                ...
                if last is not None and now - last < self.config.claude_usage_retry_seconds:
                    return False
            self._usage_running.add(key)
            self._usage_attempts[key] = now
        self.executor.submit(self._refresh_usage, tool, slot)
        return True
    ...
    def overview(self, fresh: bool) -> dict[str, Any]:
        ...
        for key in keys:
            ...
            self.maybe_refresh_usage(key[0], key[1], status, force=fresh)
    ...
    def finalize(self, tool: str, slot: str) -> str | None:
        ...
        if self.config.allowed_emails and not status.get("email"):
            # An account the list cannot be checked against must not stand either.
            with contextlib.suppress(CliError):
                self.accounts[tool].logout(slot)
            self.cache.invalidate(key)
            return "the CLI did not report which account signed in, so it was signed out again"
        if tool == "claude":
            with contextlib.suppress(OSError):
        ...
        self.maybe_refresh_usage(tool, slot, status, force=True, after_login=True)
        return None
            ...
            if match := re.fullmatch(rf"/v1/accounts/{_TOOL_SLOT}/login", route):
                ...
                if method == "POST":
                    ...
                    if session is None:
                        return _problem(HTTPStatus.NOT_FOUND, "login_not_found", "login not found")
                    return HTTPStatus.OK, session.view(self.clock())
            elif match := re.fullmatch(rf"/v1/defaults/({_TOOLS})", route):
                if method == "PUT":

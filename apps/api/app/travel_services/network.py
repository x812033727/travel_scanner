        ...
        for _ in range(6):
            ...
            async with client.stream(
                ...
                if response.status_code != 200:
                    return "unconfirmed"
                same_path = parsed.path.rstrip("/") == original.path.rstrip("/")
                if link.provider == "klook":
                    from app.travel_services.channels import (
                        klook_product_target,
                        same_klook_identity,
                    )

                    # Klook's own generator emits a short property path that redirects
                    # to /hotels/detail/ID-name. Only the same typed numeric ID qualifies.
                    try:
                        klook_product_target(link.url, "hotel")
                        same_path = same_klook_identity(link.url, current)
                    except ValueError:
                        same_path = False
                if not same_path or not all(
...
async def verify_link(
    ...
    from app.travel_services.channels import klook_canonical_target, same_klook_identity

    current = safe_url(url)
    expected = brand_target(code, expected)
    if klook_affiliate_id:
        expected = klook_canonical_target(expected)
        klook_canonical_target(current, klook_affiliate_id)
        if parse_qs(urlsplit(current).query).get("aid") != [klook_affiliate_id]:
            return False
    ...
    async with asyncio.timeout(25), httpx.AsyncClient(trust_env=False, timeout=8) as client:
        for _ in range(6):
            ...
            async with client.stream(
                ...
                if klook_affiliate_id:
                    return response.status_code == 200 and same_klook_identity(
                        expected, klook_canonical_target(current, klook_affiliate_id)
                    )

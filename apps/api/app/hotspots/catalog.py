...
CATEGORY_CORRECTIONS: dict[str, str] = {
    # Empty on purpose. Every one of the 68 entries this held was patching a category the
    # bootstrap files got wrong, and they got them wrong because the categories were handed
    # out three per city per kind rather than chosen. The files now say what each place is,
    # so a name belongs here only when two people could reasonably read the same place two
    # ways — not when the data is simply wrong. Fix the data.
}

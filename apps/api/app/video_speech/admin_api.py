...
tokens there. ``speech_router`` is what the local pipeline calls with such a token, through the
dedicated web route apps/web/app/api/video/speech (nginx exposes only the web app).
"""
...
async def create_video_tool_token(
    ...
    row = VideoToolToken(
        id=uuid4(),
        name=payload.name.strip() or "影片工具",

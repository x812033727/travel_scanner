...
def upgrade() -> None:
    ...
    # from 0015 does not. Inspecting keeps both paths safe and leaves offline SQL
    # generation unchanged.
    inspector = None if context.is_offline_mode() else sa.inspect(op.get_bind())
    has_day_settings = (
        inspector is not None and inspector.has_table("trip_route_day_settings")
    )
    has_segments = inspector is not None and inspector.has_table("trip_route_segments")

    if not has_day_settings:
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["trip_plan_id"], ["trip_plans.id"], ondelete="CASCADE"
            ),
            sa.PrimaryKeyConstraint("id"),
            sa.UniqueConstraint(
                "trip_plan_id", "day_date", name="uq_trip_route_day_setting"
            ),
        )
        op.create_index(
    ...
    if not has_segments:
        op.create_table(
            ...
            sa.Column(
                "requested_departure_time", sa.DateTime(timezone=True), nullable=True
            ),
            ...
            sa.CheckConstraint(
                "duration_minutes > 0", name="ck_trip_route_segment_duration"
            ),
            sa.CheckConstraint(
            ...
            sa.ForeignKeyConstraint(
                ["from_item_id"], ["trip_plan_items.id"], ondelete="CASCADE"
            ),
            sa.ForeignKeyConstraint(
                ["to_item_id"], ["trip_plan_items.id"], ondelete="CASCADE"
            ),
            sa.ForeignKeyConstraint(
                ["trip_plan_id"], ["trip_plans.id"], ondelete="CASCADE"
            ),

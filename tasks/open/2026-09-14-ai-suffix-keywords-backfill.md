...
title: 字尾關鍵字補進 49 篇既有 AI 文章的 description 與導言
status: review
priority: P2
area: docs
...
# 字尾關鍵字補進 49 篇既有 AI 文章的 description 與導言

## Why

`docs/ai-suffix-keywords.md` 對照出 49 篇已經寫好的文章是「〇〇 AI」字尾詞的落點，但它們的 description 與導言
...
但它沒有自然的字尾型搜尋詞（讀者打的是「AI SEO」「GEO」），拿掉了，所以是 49 篇不是 50。

## Definition of done

- [x] 49 篇的 `description` 含該篇字尾詞、120–200 字；導言第一段含該詞；`docs/ai-suffix-keywords.md`「狀態怎麼看」那段檢查全部 `ok`。
...
加 `docs/ai-suffix-keywords.md`「狀態怎麼看」的 python 檢查。部署後在主機以 `--slug` 限定這 49 篇匯入：
...
- 49 篇的 description 改寫成第一句帶字尾詞、120–200 字；導言各加一句（`chatgpt-for-excel-formulas` 改成把字尾詞寫進原句，
  因為加一句會讓正文超過 6,000 字的 lint 警戒線）。`gemini-for-travel-planning` 多一個 `link` 到 `/zh-TW/guides`。
- 檢查：49 篇 description 與導言都含該篇字尾詞；`guides-pack lint --kind life --catalogue` exit 0，warning 與改前相同
  （text_length 51 個、pack_not_in_catalogue 不變、sitemap_budget 886）；`pytest tests/test_guides_content_pack.py tests/test_gemini_series.py`
  綠；`npm run test:tools` 52 個全過；`npm run check:tasks` 只剩既有警告。
- 部署後在主機用 `--slug` 限定這 49 篇 `guides-import --dry-run`，再 `--publish`。

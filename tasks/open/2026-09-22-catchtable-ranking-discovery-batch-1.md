...
設計、邊界與站主待決事項在 `docs/catchtable-ranking-discovery.md`；操作步驟是 skill
`.agents/skills/catchtable-discovery/SKILL.md`。這張票是**第一批**：不改程式，用
...
- [ ] `apps/api/app/foods/data/catchtable/<batch-id>/` 有 `rankings.json`、`candidates.json`、
      `merchants.json`、`platform-reviews.json`，`build_batches.py --check` 零錯誤，兩個匯入檔
      各自過 repo 解析器（`load_trend_merchants`、`load_review_file`）。
...
- [ ] 候選檔保留成之後 Jev 影子量測的基準（票 `2026-09-22-catchtable-import-and-jev`）。

## Steps

- [ ] 和站主定範圍。建議：最佳餐廳榜的首爾區前 20 家加候位榜前 10 家；家數由站主定。
- [ ] 本機瀏覽器：兩個榜頁各跑一次 `references/collect-in-browser.md` 的片段，對過家數與名次，存 `rankings.json`。
...
- [ ] 本機瀏覽器逐店：店頁（韓文名、地址、hreflang、訂位控制項）＋官方來源（VisitSeoul 店家頁、
      VisitKorea、區廳名錄、官網），照 `references/candidate-file.md` 填 `candidates.json`，每家寫完就存。
- [ ] 換人抽三分之一的 `import` 重開店頁與官方頁。
- [ ] `build_batches.py --merchants-out`；本機用 `load_trend_merchants` 再驗；PR 一（候選檔、merchants.json、報告初稿）。
- [ ] 合併部署後主機 dry-run → 站主同意 → `import-trend-merchants --file … --apply`。
- [ ] 主機再匯出 worklist（拿新 merchant_id）→ `build_batches.py --worklist … --platform-out`；本機用
...
python ../../.agents/skills/catchtable-discovery/scripts/build_batches.py --candidates app/foods/data/catchtable/<batch-id>/candidates.json --check
...
- `import-trend-merchants` 的稽核來源會標 `trend-merchant-sweep`；第二階段的專用匯入器落地後改用它，
  這批不等。

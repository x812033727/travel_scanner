...
- [ ] Level-3 headings carry a stable id.
- [ ] Every existing `#section-N` anchor resolves to byte-identically the same heading.
- [ ] The visible table of contents still lists level-2 headings only.

## Steps

- [ ] In the heading branch of `ContentBlocks`, add a sub-counter that resets on each level-2,
      giving ids of the form `section-3-2`.
- [ ] The sub-counter must hang off the existing global section number, not be a second
...
- [ ] Gate it on the same `headingStart === undefined` check, so legal pages keep id-free headings.
- [ ] Leave `guideHeadings` alone: the contents list stays level-2 only, or it stops being the

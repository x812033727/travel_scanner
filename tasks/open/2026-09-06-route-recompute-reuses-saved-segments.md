...
- [ ] 整天重算時，已存在、mode 與 preference 都吻合、未過期、非 failed 的路段直接重用，不打
      provider；只有缺的段（被作廢的）才打。
- [ ] 只改緩衝 → 0 次 provider，`ready_time` 跟著新緩衝重推。
- [ ] 改當日預設交通方式 → 只有 mode 不同的非 override 段重打；`is_override` 的段沿用自己的
      mode 不重打。
- [ ] `refresh=True` 仍全部重打、跳過 Redis。
- [ ] `PUT /itinerary` 有作廢路段時不再 enqueue 整趟重算；`routing.status` 設為 `stale`，
      `total` 反映段數。第一次產生行程的 enqueue（`router.py:820、1695、3273、3463`）不變。
- [ ] `apps/api` 的 ruff / mypy / pytest 通過；整合測試裡的 auto_compute 案例
      （`test_integration_postgres_redis.py:1007-1018`）依新行為調整。

## Steps

- [ ] `route_tasks.py`：manual override 分支之後、`service.compute` 之前加「重用」分支。
- [ ] `router.py:2617-2633`：拿掉 enqueue，改寫 `routing` 狀態為 `stale`。
- [ ] `test_trip_route_tasks.py`：3 段已存刪 1 段 → provider 1 次；只改 buffer → 0 次；
      改 mode → 非 override 全打、override 不打；`refresh=True` → 全打。
- [ ] `test_integration_postgres_redis.py`：更新「編輯後會排程」的斷言。

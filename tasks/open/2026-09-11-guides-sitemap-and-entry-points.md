...
title: Wire /guides into the footer and destination pages
status: open
priority: P2
area: web
owner:
...
# Wire /guides into the footer and destination pages
...
The sitemap piece was done separately in `2026-09-11-guides-sitemap` (PR #401), so this task
now covers the other three, and its title and scope were narrowed to match.

## Definition of done

- [x] The sitemap lists the guide hubs and every published article, each only in the locales
      where it is published, and an expired notice drops out of it. (`2026-09-11-guides-sitemap`)
...
- [x] Sitemap entries from `GET /guides/sitemap`, with cases in `app/sitemap.test.ts`. (PR #401)
...
- The two notes above are about the sitemap piece, kept as history: PR #401 did both, and
  `app/sitemap.ts` / `app/sitemap.test.ts` left this task's scope on 2026-09-12
  (claude-opus-5-guides-sitemap, as agreed with the user).

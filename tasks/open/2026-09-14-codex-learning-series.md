...
**2026-09-20 發布更新：** 60 篇五語全文終審與總目錄已完成，PR #578 與查詢網址修正 PR #583 已合併，305 份語言文件已正式發布，綠燈版本 `6532eaa6e7e9d5a34b94cfe8f8a62ed2e0fba9a1` 已部署並通過公開瀏覽器驗收；見 `docs/codex-learning/production-release-2026-09-20.md`。以下「0/60」「未開 PR／部署」是歷史檢查點，已由此更新取代。本任務仍保留未完成的代表篇 Codex 產品介面實際操作證據；已公開文章明確區分原創示意圖、練習網站截圖與依官方文件查證的平台步驟，沒有把未實測裝置說成已實測。
...
後續已實際補強 03／04／10／11／23 五篇及五語內容包，加入副本比對、Remote 主機辨識、規則適用條件、CLI 續接與 Skills 缺檔恢復。定向審核和教材結果見 docs/codex-learning/evidence/representative-depth-review.json；9 項 API 內容測試、9 項編譯器測試與全系列完整性通過，稽核為 0 錯誤／25 提示。全篇終審、產品證據及最終頁面仍待完成，不更新為已驗收。
...
- [x] 60 篇＋目錄最終五語整頁預覽：360／390／1280px、複製、搜尋分享、語言切換、前後篇、正文引用與圖文。
- [x] 完成內容與頁面驗收後，對齊 main、審查精確差異、開 PR、確認 exact head CI 並合併。
- [x] 依使用者既有「全部完成後再開 PR 部署」授權，備份後匯入／正式發布／部署，核對公開五語網址。無須另把已授權步驟當成尚未授權。

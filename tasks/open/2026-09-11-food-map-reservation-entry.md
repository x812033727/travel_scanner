...
PR391 reconciles main 5e3168e1. Local rerun: 18 Vitest files /179 tests, discovery API56, full typecheck/lint and scoped Ruff passed. Actual browser CI exposed a repeated-journey native dialog bug: run34563980545 records final `cancel -> back -> close -> back` for the same food detail, navigating to about:blank. The scoped drawer now consumes Escape before the browser default and deduplicates dismissals until the URL changes; five regression cases cover native event pairs, rapid taps, direct URLs, child modals and IME/consumed keys (11 drawer tests pass). Diagnostic history monkey-patching was removed; exact URL, focus, filters and light/dark repeated journeys remain asserted, with failure traces retained. The updated exact head must still pass CI before merge.

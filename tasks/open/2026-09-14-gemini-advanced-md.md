...
- [x] 73：複驗 Windows 原生 gemini extensions 命令生命週期。2026-09-14 追加 Node 22.23.2：原版 CLI 入口與 Windows PowerShell 5.1 的 npm gemini.cmd 各十四步通過；後者另驗檔案副本與工作區停用／啟用。Node 24.13.0 安裝、24.19.0 停用仍重現 3221226505，教材已保留版本限制。輸出重導紀錄不代表可視終端或模型會話驗證。
...
### 2026-09-14 Windows 原生入口追加驗證

本次不是重新產製六篇：補第 73 篇的 Node 版本實測與更新確認步驟。正文 2,954 字；六篇範圍為 2,282–2,954 字。verify-native.py 不改 CLI、無模型呼叫，獨立暫存 home 完整移除；失敗或逾時停止後續寫入，保留結束碼與當下副本。PowerShell 成功包含工作區 false／true、更新到 1.1.0、回退到完整 1.0.0。原生 Node 24 兩個崩潰仍保留，沒有把非零結束码當成功。

新增三版本 20 筆觀察與 PowerShell 14 筆；前次 48 筆模組觀察仍獨立保存。七包 ZIP 重新核對 CRC／成員內容，綜合包解壓後重跑 48 筆觀察通過；綜合包加入 native 重做工具與紀錄。內容包重建、六篇 draft check、Python Ruff 通過。配圖本體未改；圖說改成特定 Node 22 組合通過、Node 24 仍異常。

原作者快照保存在 authoring-review-initial-20260914.json；當前 authoring-review.json 為 revision 2，重新核對所有被引用檔案雜湊。仍未發布，72／73 的模型回答／技能觸發與 74 會話恢復未驗；不勾整批發布完成。

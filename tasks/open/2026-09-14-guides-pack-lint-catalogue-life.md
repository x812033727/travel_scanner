...
- [ ] `--catalogue` 只在「總表有、內容包沒有」時報缺漏；內容包不在總表裡不再報警告，
      或改成一行摘要（例如「另有 N 篇 life 文章不屬於這張總表」）而不是每篇一行。
- [ ] `tests/test_guides_pack_ingest.py` 加一個案例：總表外的 life 內容包不產生 per-slug 警告。
- [ ] 跑 `lint --kind life --catalogue ../../docs/life-ai-series.md`，輸出只剩真正的缺漏。

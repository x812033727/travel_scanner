...
- [ ] 用型別不符的 `search_id` 呼叫 flight-sources/expand，得到 4xx 與可讀訊息，而不是 500。
- [ ] 用 `result_json` 中含有無法解析的航班報價的列呼叫，同樣不會 500。
- [ ] 回歸測試涵蓋這兩種情況。

## Steps

- [ ] 在 `_owned_search()` 或 `expand_flight_sources` 內判斷 `operation`，非航班搜尋的列以
      `AppError(404/409)` 拒絕；沿用 `:198-201` 既有的 try/except 形式包住兩個 `model_validate`。
- [ ] 同檔案 `:197`、`:358`、`:409` 有同樣的裸 `FlightOffer.model_validate` 形式，一併檢視。
- [ ] 新增 `apps/api/tests/test_search_flight_sources.py`：植入一列

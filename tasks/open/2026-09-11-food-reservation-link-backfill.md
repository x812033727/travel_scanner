...
- 還沒確認的一筆：台南翰林茶館赤崁店的 inline 品牌頁。多次重試都被 inline 的「按住不放」驗證擋下
  （沒有破解）。等封鎖過去或由人在一般瀏覽器開啟確認後，把資料檔那筆改成 `verified` 或 `disabled`，
  再跑一次匯入指令即可，不必重跑整輪查核。翰林在 OpenRice 另有本分店頁面，只能電話訂位，已記為停用。

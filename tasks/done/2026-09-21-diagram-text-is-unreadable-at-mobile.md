...
## 2026-09-21 續修：可視窗口撐到整個螢幕寬

第一版上線後站主指出第三欄被切掉。量了座標才發現問題比看到的大：
那張圖**三欄各是 490 畫布單位 → 畫面上 361px**，而盒子只有 335px，
所以**三欄沒有一欄看得完整**，只是前兩欄被切的是右側、比較不明顯。

**縮小圖是死路。** 掃過全站 1004 張圖，最寬的內容方框到 1300 畫布單位；
要讓每個方框都塞進 335px，圖只能畫到 412px，最小的字變 3.9px，比沒修還糟：

| 想涵蓋的比例 | 方框上限 | 圖只能畫到 | 15px 標籤變成 |
|---|---|---|---|
| 50% | 420 | 1276px | 12.0px |
| 75% | 640 | 838px | 7.9px |
| 100% | 1300 | 412px | 3.9px |

**做法是加寬窗口，不是縮小圖。** 文章的左右留白來自 `main` 的 `px-5`，
**固定 20px 且所有斷點都一樣**（`main` 是 `mx-auto px-5 py-10 md:py-14 max-w-3xl`；
`max-w-3xl` 是 768px，扣掉 40px 就是量到的 728px 欄寬）。
所以捲動盒用 `-mx-5` 抵銷它、再用 `px-5` 加回盒子內部：

| | 續修前 | 續修後 |
|---|---|---|
| 可視窗口（手機） | 335px | **375px** |
| 一欄 361px | 放不下 | **放得下** |
| 圖寬與字級 | 1180px / 11.1px | 不變 |
| 圖的左緣 | 對齊正文 | 對齊正文，不變 |

留白加回盒子內部，是為了讓圖在還沒滑動時左緣仍對齊正文，而右緣延伸出螢幕正好當作
「還有」的提示。桌機同步從 728 變成 768（等於 `main` 本身的寬度），置中、沒有溢出。

**不要用 `100vw`。** 它含垂直捲動條寬度，會撐出整頁橫向捲動，違反這張票自己的第三條。
`-mx-5` 之所以安全，正是因為它抵銷的是一個已知的固定值，不是視窗寬度。

實測：手機 375px 與桌機 1600px 兩種視窗，頁面都沒有橫向捲動；
第三欄「꼬리곰탕 牛尾湯」連同三個方框完整入鏡。

## 2026-09-22 收尾：右內距拿掉

站主在正式站上看到，捲到最底會停在 20px 的空白上，最後一欄的前緣反而被推出畫面。
原因是捲動盒左右各給了 20px 內距，**只有左邊那個有用**（讓圖在還沒滑動時對齊正文），
右邊那個純粹是對稱寫法，代價是捲動終點落在空白上。

`px-5` → `pl-5`，`xl:px-0` → `xl:pl-0`。

| | 改前 | 改後 |
|---|---|---|
| 捲動內容總寬 | 1220px | 1200px |
| 最大捲動量 | 845px | 825px |
| 捲到底時圖右緣與盒子右緣的距離 | 20px | **0** |

正式站實測（375px）：`imgRight` 與 `boxRight` 都是 375。
`xl` 不受影響，本來就沒有捲動。

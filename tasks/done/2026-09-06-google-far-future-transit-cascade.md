---
id: 2026-09-06-google-far-future-transit-cascade
title: Google 遠期大眾運輸一段最多打 6 次，且是唯一沒有預算保留的路線 provider
status: done
priority: P2
area: api
owner: claude-opus-5
claimed_at: 2026-09-06T10:12:58Z
created_at: 2026-09-06T02:45:25Z
completed_at: 2026-09-06T10:23:25Z
branch: claude/route-honesty
depends_on: []
scope:
  - apps/api/app/trips/routing.py
  - apps/api/tests/test_trip_routing.py
---

# Google 遠期大眾運輸一段最多打 6 次，且是唯一沒有預算保留的路線 provider

## Why

`GoogleRouteProvider.compute_options`（`routing.py:511-578`）為一段路準備最多 6 個請求 body
（`requested`、`without_preference`、`coordinates`、`near_term_schedule`、`near_term_daytime`、
`current_schedule`），空結果就打下一個。出發時間超過 14 天的大眾運輸，`requested` 幾乎必空
（Google 只公布近期班次），所以一段路常常是「2–4 次計費換 1 個結果」。

而且 Google 是唯一沒有 `reserve_*` 預算保留的路線 provider：`usage_meter.py:314` 有
`reserve_google_maps_request`，但 `_post` 只在 `finally` 裡 `record_google_maps_request`，
永遠不會擋。

影響範圍：Google 負責大眾運輸的地區（台灣等；日本走 Ekispert、韓國走 ODsay），以及所有地區的
步行／汽車。

## Definition of done

- [x] 出發時間 > now + 14 天的大眾運輸，第一個送出的請求就是 `near_term_schedule`，
      其餘 fallback 順序不變。
- [x] Google 路線請求在送出前先 `reserve_google_maps_request`，額度用完時回 `[]` 並記 log，
      跟 Ekispert／ODsay 一致。
- [x] 現有 `test_scheduled_transit_*` / `test_far_future_transit_*` 測試依新順序調整，
      並新增「>14 天只打一次」的案例。

## Steps

- [x] 讀 `supported_transit_time`（`routing.py:242`）與 `_next_matching_transit_time`
      確認 14 天門檻的來源。
- [x] 調整 `attempts` 的組裝順序。
- [x] 接 `reserve_google_maps_request`（看 `restaurants/google.py:162,214` 的用法）。

## How to verify

```bash
cd apps/api && uv run pytest tests/test_trip_routing.py -k "google"
```

## Notes

- 從 2026-09-06 交通成本檢討分出來；主要漏額度的是整趟重算與快取 key，先修那兩張
  （`2026-09-06-route-cache-time-key`、`2026-09-06-route-recompute-reuses-saved-segments`）。

2026-09-06 claude-opus-5：兩件事都做了。

- 順序：`compute_options` 組完 attempts 之後，若是 scheduled 的大眾運輸且 `effective_time > now + 14 天`，
  把 `near_term_schedule` 那筆搬到第一個，其餘順序不動（`attempts.insert(0, attempts.pop(index))`）。
  超過 100 天的行程不受影響，因為 `supported_transit_time` 早就把它換成 preview 時間，本來第一筆就是近期參考。
- 預算：新增 `GoogleRouteProvider._reserve()`，用 `reserve_google_maps_request(redis, "routes",
  settings.google_maps_essentials_free_limit)`，在 `_post` 送出前保留；用完丟 `GoogleRoutesBudgetExceeded`，
  `compute_options` 記 `monthly_budget_exhausted` 並回 `[]`，`probe` 回 `MONTHLY_BUDGET_EXHAUSTED`，
  跟 NAVITIME／Ekispert／ODsay 一致。原本 `finally` 裡的 `record_google_maps_request` 拿掉，改由 reserve 計數，
  沒有雙重計算。沒有新增後台設定：routes 屬 Essentials SKU，直接用既有的免費額度欄位當上限。
- Redis 讀不到時 `reserve_*` 回 False（fail closed），這點與其他 provider 相同：寧可不打，也不要送出算不到的付費請求。
- 測試：`test_scheduled_transit_without_published_timetable_uses_near_term_preview` 改名為
  `test_transit_beyond_published_timetables_asks_the_near_term_reference_first`，斷言只送一次請求、
  第一筆就是近期同星期同時段且不帶 transitPreferences；另外兩支遠期測試的請求數與順序不變（各 5、3 次）；
  新增 `test_google_routes_stop_at_the_free_tier_and_report_it_to_the_admin`。

...
第一輪只查了 2026-09-11 留下的候選頁：83 間仍無連結的日本店家裡，ホットペッパー 只涵蓋 20 間、
ぐるなび 13 間、AutoReserve 11 間。第二輪對這 83 間各派一個代理，在五個平台網域上找店頁並判斷
是否同一分店（**不判斷能不能訂位**，那一步全部機械化），回報 259 個候選頁。

結果：再多 5 間可訂位，全在 AutoReserve，而且五個 id 都和 2026-09-11 記下的不同——
第一輪漏掉就是因為當時記的是別的 id。ホットペッパー 65 頁、ぐるなび 66 頁全數落空。
日本覆蓋率 10/102 → 24/102。PR 見 `2026-09-12-japan-platforms-second.md`。

有用的技巧：ぐるなび 擋一般抓取，但**在 `r.gnavi.co.jp` 的任一頁上用同源 `fetch('/{店家id}/')`
可以一次取回幾十頁**，判斷條件是回應內文含 `plan-reserve/plan/plan_list`。這招對四個已知答案完全對上，
比逐頁開瀏覽器快兩個數量級。

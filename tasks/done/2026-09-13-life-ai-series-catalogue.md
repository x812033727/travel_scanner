...
- [x] `docs/life-ai-series.md`：目的與讀者、語系（zh-TW）、配圖與合作連結政策、十一個批次與任務票、
      220 篇的 slug／標題／topics／圖／合作／易變、產製流程、經驗記錄區。
- [x] 所有 slug 符合 `SLUG_PATTERN`、與現有 50 個旅遊 slug 不重複、topics 只用 life 詞彙。

## Steps

- [x] 寫總表。
- [x] 用 `guides-pack lint --catalogue` 對一次（工具票完成後）。
...
- `docs/life-ai-series.md`：十一批 220 篇，slug 對過 `SLUG_PATTERN`、與 50 個旅遊 slug 不重複、topics 只用 life 詞彙。
- `guides-pack lint --kind life --catalogue docs/life-ai-series.md`：批次 01 的二十篇已寫，其餘 200 篇列為 `catalogue_missing_pack`。

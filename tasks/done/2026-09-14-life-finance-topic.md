...
area: api
owner: claude-opus-5
claimed_at: 2026-09-14T12:19:25Z
created_at: 2026-09-14T11:45:30Z
completed_at: 2026-09-14T12:32:46Z
...
- [x] `apps/api/app/guides/taxonomy.py` 的 `LIFE_SEED_TOPICS` **末尾附加**一列（不是插在中間）：
      `("finance", ("Money & finance", "お金・家計", "돈・재테크", "理財與金錢", "理财与金钱"))`。
      五語系標籤齊全（`seed_names` 對 `LOCALES` 用 `strict=True`）。
- [x] 新增 `apps/api/migrations/versions/0075_finance_topic.py`，`down_revision = "0074_lifestyle_guides"`。
- [x] `uv run alembic heads` 只有一個 head；upgrade → downgrade → upgrade 都過。
- [x] `apps/web/lib/guide-affiliate.test.ts` 裡列出七個 life slug 的那一行加上 `finance`（順手，不是必要）。
- [x] `test_guides_migration.py`、`test_guides.py`、`test_guides_content_pack.py` 全綠。
...
- [x] `if context.is_offline_mode(): return` 擋在 seed 前面。
- [x] 先 `SELECT slug FROM guide_topics`，**只插入不存在的 slug**——重跑是 no-op，
      也不覆寫管理員改過的東西（0072 的契約）。
- [x] seed 清單的五語系標籤**寫在 migration 檔案裡，不要從 `app.guides.taxonomy` import**。
      0074 也是這樣：migration 必須在應用常數之後再改時仍然跑得動。
- [x] 欄位：`section="life"`、`source="seed"`、`is_active=True`、`display_order=270`（0074 用到 260）、
      `uuid4()` id、`datetime.now(UTC)`。
- [x] `downgrade()` 先刪 `guide_article_topics` 的關聯再刪 topic（SQLite 沒有 pragma 時不串聯）。
      **條件是 `slug = 'finance' AND source = 'seed'`，不是 `section = 'life'`**——
      0074 刪的是全部 life seed 主題，這裡照抄會把 AI 系列的七個主題一起清掉。
- [x] 沒有新欄位、沒有 `batch_alter_table`，所以沒有 `if "x" not in columns` 分支，
...
- [x] 把它改成把 0074 與 0075 的 seed 清單**依序串起來**再比對，例如
      `(*m0074.LIFE_SEED_TOPICS, *m0075.FINANCE_SEED_TOPICS)`。
      依序串接只有在 `taxonomy.py` 是**附加**而不是插入時才成立——這就是上面要求附加的原因。
- [x] 新增一個測試跑 0074 → 0075：`finance` 以 `section="life"`、`display_order=270` 落地；
      重跑 0075 是 no-op；downgrade 先移除關聯再移除 topic，且**不影響 0074 的七個主題**。
- [x] `test_guides.py` **要改一行**：`test_topics_belong_to_one_section_and_are_refused_in_the_other`
      把 life 的七個 slug 依序寫死在斷言裡，附加 `finance` 會讓它紅，末尾補上 `"finance"` 即可。
      （`test_the_two_seed_vocabularies_stay_disjoint` 對 tuple 是泛用的，不用動。）
      這個檔案雖然也在 `2026-09-12-guide-topic-admin-crud` 的 scope 裡，但那張票是 `open`
      且沒有 owner，`HOLDS_SCOPE` 只認 in-progress 與 review，所以不互卡。
...
`finance` 已進 `LIFE_SEED_TOPICS`（附加在 `misc` 之後）與新的 `0075_finance_topic`，
`display_order=270`。`_known_topics('life')` 現在認得 `finance`，批次票可以開工。

**一個規劃時判斷錯的地方**：原本以為 `apps/api/tests/test_guides.py` 不用改。
實際上 `test_topics_belong_to_one_section_and_are_refused_in_the_other` 把 life 的七個 slug
**依序寫死**在斷言裡，附加 `finance` 會讓它紅。已在末尾補上 `"finance"` 並加註解說明
新詞彙是附加的。那個檔案雖然在 `2026-09-12-guide-topic-admin-crud` 的 scope 裡，
但那張票是 `open` 且沒有 owner，`HOLDS_SCOPE` 只認 `in-progress` 與 `review`，所以不互卡；
已把它加進這張票的 scope。

另外補了一個 `test_the_seeded_life_display_orders_do_not_collide`：兩個 migration 的
display_order 不重複且遞增。附加而非插入是整個設計的前提，這條把它釘住。

**驗證**：`test_guides_migration.py`、`test_guides.py`、`test_guides_content_pack.py`、
`test_guides_pack_ingest.py`、`test_migration_sql_dialect.py`、`test_migration_dead_branches.py`
共 109 passed；`ruff`、`mypy` 全綠；`npm run test:web` 258 個檔案 2,809 個測試全過。
`alembic heads` 只有 `0075_finance_topic`；offline SQL 產生器確認 `is_offline_mode` 分支
不會輸出 INSERT。

**這個容器沒有 PostgreSQL**，所以 `alembic upgrade head` 跑不起來（CI 會跑）。
真正驗證升級／重跑／回滾行為的是新加的 SQLite 測試。

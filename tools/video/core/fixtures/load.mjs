...
/** A fake repository holding the fixture as docs/videos/<slug>/, and a work base beside it. */
export function sandbox(slug = "fixture-minimal") {
  const base = mkdtempSync(path.join(tmpdir(), "video-core-"));
  const root = path.join(base, "repo");
  const videos = path.join(root, "docs", "videos");
  mkdirSync(videos, { recursive: true });
  cpSync(path.join(FIXTURES, "minimal"), path.join(videos, slug), { recursive: true });
